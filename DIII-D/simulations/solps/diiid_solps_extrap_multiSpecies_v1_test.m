...
mpfx = linspace(2.14333, 2.20675, 1000);   mpfy = 0*mpfx;
...
K_edge = 5;       % use last K points for robust edge value
...
idx_ne = find(ok_ne_mask(:)); idx_Te = find(ok_Te_mask(:)); idx_v = find(ok_v_mask(:));
coords_ne = coords_ok_ne(idx_ne,:); vals_ne = neS_clamp(idx_ne);
coords_Te = coords_ok_ne(idx_Te,:); vals_Te = TeS_clamp(idx_Te);
coords_v  = coords_ok_ne(idx_v,:);  vals_vr = vrS_clamp(idx_v); vals_vz = vzS_clamp(idx_v); vals_vt = vtS_clamp(idx_v);
...
parfor b = 1:nBatches
    ...
    dN   = vecnorm(coords_ne(idxN,:) - subset, 2, 2);
    ne_out = vals_ne(idxN) .* exp(-dN/Ldecay);

    % Te
    idxT = knnsearch(coords_Te, subset, 'K', 1);
    dT   = vecnorm(coords_Te(idxT,:) - subset, 2, 2);
    Te_out = max(vals_Te(idxT) .* exp(-dT/Ldecay), T_min);

    % velocities
    idxV = knnsearch(coords_v, subset, 'K', 1);
    % OLD: slow/none decay via Ldecay_v
    % dV   = vecnorm(coords_v(idxV,:) - subset, 2, 2);
    % vr_out = vals_vr(idxV) .* exp(-dV/Ldecay_v);
    % vz_out = vals_vz(idxV) .* exp(-dV/Ldecay_v);
    % vt_out = vals_vt(idxV) .* exp(-dV/Ldecay_v);

    % NEW: constant (nearest) velocities in Euclidean extrapolation
    vr_out = vals_vr(idxV);
    vz_out = vals_vz(idxV);
    vt_out = vals_vt(idxV);
...
for k = 1:ns
    ...
    % Euclidean propagation to (X,Y)
    ok_mask_ni = isfinite(ni_ex) & ni_ex>0;
    ok_mask_Ti = isfinite(Ti_ex) & Ti_ex>0;
    ok_mask_u  = isfinite(uR_ex) | isfinite(uZ_ex) | isfinite(uT_ex);

    idx_ni = find(ok_mask_ni(:)); idx_Ti = find(ok_mask_Ti(:)); idx_u = find(ok_mask_u(:));
    if isempty(idx_ni)
        warning('Species k=%d has no finite ni points; skipping.', k); continue;
    end
    coords_ok = [RRs(idx_ni), ZZs(idx_ni)];
    vals_ni   = ni_ex(idx_ni);
    coords_T  = [RRs(idx_Ti), ZZs(idx_Ti)]; vals_Ti = Ti_ex(idx_Ti);

    coords_u  = [RRs(idx_u),  ZZs(idx_u)];
    vals_uR   = uR_ex(idx_u); vals_uZ = uZ_ex(idx_u); vals_uT = uT_ex(idx_u);

    ni_cell = cell(nBatches,1); Ti_cell = cell(nBatches,1);
    uR_cell = cell(nBatches,1); uZ_cell = cell(nBatches,1); uT_cell = cell(nBatches,1);
    ...
    parfor b = 1:nBatches
        ...
        ni_cell{b} = vals_ni(iN) .* exp(-dN/Ldecay);

        iT = knnsearch(coords_T, subset, 'K', 1);
        dT = vecnorm(coords_T(iT,:) - subset, 2, 2);
        Ti_cell{b} = max(vals_Ti(iT) .* exp(-dT/Ldecay), T_min);

        iU = knnsearch(coords_u, subset, 'K', 1);
        % OLD: decayed with Ldecay_v
        % dU = vecnorm(coords_u(iU,:) - subset, 2, 2);
        % uR_cell{b} = vals_uR(iU) .* exp(-dU/Ldecay_v);
        % uZ_cell{b} = vals_uZ(iU) .* exp(-dU/Ldecay_v);
        % uT_cell{b} = vals_uT(iU) .* exp(-dU/Ldecay_v);

        % NEW: constant (nearest) velocities
        uR_cell{b} = vals_uR(iU);
        uZ_cell{b} = vals_uZ(iU);
        uT_cell{b} = vals_uT(iU);
    end
...
save('multiSpecies_data_196154_test.mat')
%% ------------------- NetCDF Export (single-fluid + multispecies) -------------------
x  = X(1,:);          % R
z  = Y(:,1);          % Z
nR = length(x); nZ = length(z);

outnc = 'profilesDIIID_196154_multi_test.nc';
