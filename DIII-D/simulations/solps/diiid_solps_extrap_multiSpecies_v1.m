...
for jz = 1:nz
    ...
    for ir = edge_idx+1:nr
        ...
        vrS_clamp(ir,jz)  = e_vr  * fac;
        vzS_clamp(ir,jz)  = e_vz  * fac;
        vtS_clamp(ir,jz)  = e_vt  * fac;
...
parfor b = 1:nBatches
    ...
    vr_out = vals_vr(idxV) .* exp(-dV/Ldecay);
    vz_out = vals_vz(idxV) .* exp(-dV/Ldecay);
    vt_out = vals_vt(idxV) .* exp(-dV/Ldecay);
...
for k = 1:ns
    ...
    for jz = 1:nz
        ...
        for ir = edge_idx+1:nr
            ...
            uR_ex(ir, jz) = e_uR * fac;
            uZ_ex(ir, jz) = e_uZ * fac;
            uT_ex(ir, jz) = e_uT * fac;
        end
    end
    ...
    parfor b = 1:nBatches
        ...
        uR_cell{b} = vals_uR(iU) .* exp(-dU/Ldecay);
        uZ_cell{b} = vals_uZ(iU) .* exp(-dU/Ldecay);
        uT_cell{b} = vals_uT(iU) .* exp(-dU/Ldecay);
    end
...
if any(isC)
    kc = find(isC,1,'first');
    figure; imagesc([min(X(:)) max(X(:))],[min(Y(:)) max(Y(:))], val_ni_mspecies(:,:,kc));
