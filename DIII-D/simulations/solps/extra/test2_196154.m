...
neS     = ncread(fileSOLPS, 'ne');       % Electron density
TeS     = ncread(fileSOLPS, 'te');       % Electron temperature
...
gradTeS = ncread(fileSOLPS, 'gradTe');   % Electron‐temperature gradient
gradTiS = -ncread(fileSOLPS, 'gradTi');  % Ion‐temperature gradient (negate if desired)
%% Data Cleaning
...
num_points = 20; % Adjust resolution as needed
...
mpfx = linspace(2.281, 2.315, 1000); % DIII-D # 200882
...
mpx = linspace(2.281, 2.5, 1000); % DIII-D # 200882
...
interpfn = (mpx - 2.281) / (2.315 - 2.281); % DIII-D # 200882
