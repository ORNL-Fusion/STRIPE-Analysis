...
vars2D = {neS', TeS',  gradTeS', vrS', vzS', vtS'};
titles = {'Electron Density', 'Electron Temperature', ...
          'Electron Temperature Gradient' ...
          'Radial Velocity', 'Poloidal Velocity', 'Toroidal Velocity'};
...
okValues_gradTe = gradTeS';
okValues_gradTe = okValues_gradTe(idx);

%% Midplane Density & Temperature Extrapolation (Polynomial Fit)
mpfx = linspace(2.14333, 2.20675, 1000); % # 196154
% mpfx = linspace(2.28313, 2.31587, 1000); % # 200882
...
mpx = linspace(2.14333, 2.5, 1000); % # 196154
% mpx = linspace(2.28313, 2.5, 1000); % # 200882
...
p_gradTe = polyfit(mpfx, fitGradTiAtMidplane, 1);

% Extrapolation function
interpfn = (mpx - 2.14333) / (2.20675 - 2.14333); % # 196154
% interpfn = (mpx - 2.28313) / (2.5 - 2.28313); % # 200882
...
plot(mpx, extrapolatedGradTe1d, 'k', 'DisplayName', 'Extrapolated Grad-Te');
...
parfor batch_idx = 1:ceil(num_points / batch_size)
    ...
    val_gradTe_cell{batch_idx} = okValues_gradTe(idx_nearest) .* (1 - distances/max(distances));
end
...
val_gradTe = cell2mat(val_gradTe_cell);
...
val_gradTe = reshape(val_gradTe, size(X));
...
disp('Extrapolation complete with w/o mask applied only beyond limiter boundary.');
