...
mpx = linspace(2.14547, 2.5, 1000); % DIII-D # 196154
...
interpfn = (mpx - 2.14547) / (2.2051 - 2.14547); % DIII-D # 196154
%% 
...
%% ----- Update Velocities and gradTi Using Similar Exponential Decay Method -----
% For each vertical slice (each column), use the same approach as for density.
% For velocities and gradTi, we assume baseline = 0.
for i = 1:length(zS)
    edge_idx = find(plasma_mask(:, i), 1, 'last');  % Last valid index for density is used as proxy for plasma edge
    if ~isempty(edge_idx) && edge_idx < length(rS)
        R_edge = rS(edge_idx);
        % For velocities and gradTi, get the edge values
        ...
        for j = edge_idx+1:length(rS)
            if inside_limiter_mask(j, i)
                % Exponential decay: baseline 0
                vrS(j, i) = edge_vr * exp(-(rS(j) - R_edge) / decay_length);
                vzS(j, i) = edge_vz * exp(-(rS(j) - R_edge) / decay_length);
                vtS(j, i) = edge_vt * exp(-(rS(j) - R_edge) / decay_length);
                gradTiS(j, i) = edge_gradTi * exp(-(rS(j) - R_edge) / decay_length);
            end
        end
    end
end
...
colorbar;
title('Extrapolated Radial Velocity (vr)');
hold on;
plot(g.lim(1,:), g.lim(2,:), 'r');
...
set(gca, 'YDir', 'normal', 'ColorScale', 'linear');
colorbar;
title('Extrapolated Poloidal Velocity (vz)');
hold on;
plot(g.lim(1,:), g.lim(2,:), 'r');
...
set(gca, 'YDir', 'normal', 'ColorScale', 'linear');
colorbar;
title('Extrapolated Toroidal Velocity (vt)');
hold on;
plot(g.lim(1,:), g.lim(2,:), 'r');
...
set(gca, 'YDir', 'normal', 'ColorScale', 'linear');
colorbar;
title('Extrapolated Ion Temperature Gradient (gradTi)');
hold on;
plot(g.lim(1,:), g.lim(2,:), 'r');
