...
load('extrapolated_data_200882.mat');  % expects: X, Y, g (includes g.lim)
...
nP = 5e6;                          % Number of test particles
% source_strength = 9.442619262858435e+17; % # 196154 % particles/sec from GITRm
source_strength = 7.176454593335885e+20; % # 200882
n_Spec = 1;

%% 4) Load GITRm spec data
path1 = "../diiid-helicon/DIII-D_helicon_runs/";
...
data_trim = squeeze(data_1(:,:,1,1))';     
% data_trim = data_1(:,:,6)';   % [nZ × nR] % 6 is for carbon 6
...
%% 12) Extract and compare at midplane
% Identify midplane index (z ≈ 0)
[~, mid_idx] = min(abs(gridz_2d));   % index where z ~ 0
z_mid = gridz_2d(mid_idx);
fprintf('\n=== Midplane Comparison (z = %.3f m) ===\n', z_mid);

% GITR carbon density at midplane
density_mid = density(mid_idx, :);
r_mid = gridr_2d;


% --- Plot comparison ---
figure('Name','Midplane Carbon Comparison');
semilogy(r_mid, density_mid, 'r-', 'LineWidth', 2); hold on;
% semilogy(solps_r, solps_D_total, 'b--', 'LineWidth', 2);
% semilogy(solps_r, solps_C_total, 'g-.', 'LineWidth', 2);
xlabel('R [m]');
ylabel('Density [m^{-3}]');
title(sprintf('Midplane Densities (z = %.3f m)', z_mid));
legend({'GITR Carbon', 'SOLPS Total D', 'SOLPS Total C'}, 'Location', 'best');
grid on;
set(gca, 'FontSize', 14);
...
R = unique(gridr_2d);
Z = unique(gridz_2d);
[X, Y] = meshgrid(R, Z);    % now X, Y are 2D grids
psiN_flat = calc_psiN(g, X(:), Y(:), []);
psiN = reshape(psiN_flat, size(X));

[~, mid_idx] = min(abs(Z)); % midplane index
psiN_mid = psiN(mid_idx, :);
density_mid = reshape(density, size(X));
density_mid = density_mid(mid_idx, :);

% Define common psiN grid
psiN_common = linspace(0, 1.2, 200);

% Interpolate GITR density to common psiN
dens_gitr_interp = interp1(psiN_mid, density_mid, psiN_common, 'linear', 'extrap');

% For SOLPS
% [~, solps_mid_idx] = min(abs(g.zg - z_mid));
% solps_dens_mid = squeeze(g.dens(:, solps_mid_idx, :));
% solps_C_total = sum(solps_dens_mid(:, 3:9), 2);
% solps_D_total = sum(solps_dens_mid(:, 1:2), 2);
% 
% psiN_solps = g.psin;  % or compute with calc_psiN if missing
% dens_C_solps_interp = interp1(psiN_solps, solps_C_total, psiN_common, 'linear', 'extrap');
% dens_D_solps_interp = interp1(psiN_solps, solps_D_total, psiN_common, 'linear', 'extrap');

figure('Name','Midplane Densities vs ψ_N');
semilogy(psiN_common, dens_gitr_interp, 'r-', 'LineWidth', 2); hold on;
% semilogy(psiN_common, dens_D_solps_interp, 'b--', 'LineWidth', 2);
% semilogy(psiN_common, dens_C_solps_interp, 'g-.', 'LineWidth', 2);
xlabel('\psi_N');
ylabel('Density [m^{-3}]');
title(sprintf('Midplane Densities vs ψ_N (z = %.3f m)', z_mid));
legend({'GITR Carbon', 'SOLPS Total D', 'SOLPS Total C'}, 'Location', 'best');
grid on;
set(gca, 'FontSize', 14);