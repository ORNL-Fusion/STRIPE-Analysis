...
for it = 1:8
    ...
    % Compute metrics for all planes
    for i = 1:length(planes)
        ...
        longest_side(i) = max(side_length(i, :));
    ...
    % Identify planes to refine
    refine_inds = find(area > 0.0005 | aspect_ratio > 10 | longest_side > 0.1);
    planes_no_refine = planes;
    planes_no_refine(refine_inds, :) = [];

    % Refine selected planes
    [X_r, Y_r, Z_r] = refineXYZ(X(refine_inds, :), Y(refine_inds, :), Z(refine_inds, :), 1);
    planes_refined = [X_r(:, 1), Y_r(:, 1), Z_r(:, 1), ...
                      X_r(:, 2), Y_r(:, 2), Z_r(:, 2), ...
                      X_r(:, 3), Y_r(:, 3), Z_r(:, 3)];
...
%% Compute Final Geometry Properties
area = zeros(length(planes), 1);
side_length = zeros(length(planes), 3);
aspect_ratio = zeros(length(planes), 1);
...
%% Visualize Refined Geometry
figure;
patch(transpose(X), transpose(Y), transpose(Z), 'g', 'FaceAlpha', 1, 'EdgeColor', 'k');
title('Refined Geometry');
xlabel('X'); ylabel('Y'); zlabel('Z');

% Highlight planes with high aspect ratios
plotSet = find(aspect_ratio > 5);
X_high = [planes(plotSet, 1), planes(plotSet, 4), planes(plotSet, 7)];
Y_high = [planes(plotSet, 2), planes(plotSet, 5), planes(plotSet, 8)];
Z_high = [planes(plotSet, 3), planes(plotSet, 6), planes(plotSet, 9)];
figure;
patch(transpose(X_high), transpose(Y_high), transpose(Z_high), 'g', 'FaceAlpha', 0.5, 'EdgeColor', 'k');
title('High Aspect Ratio Planes');

% Highlight planes with lower aspect ratios
plotSet = find(aspect_ratio <= 5);
X_low = [planes(plotSet, 1), planes(plotSet, 4), planes(plotSet, 7)];
Y_low = [planes(plotSet, 2), planes(plotSet, 5), planes(plotSet, 8)];
Z_low = [planes(plotSet, 3), planes(plotSet, 6), planes(plotSet, 9)];
hold on;
patch(transpose(X_low), transpose(Y_low), transpose(Z_low), 'b', 'FaceAlpha', 0.5, 'EdgeColor', 'k');
title('Geometry with Highlighted Aspect Ratios');
...
zz = comsol_data(:, 3);
ne = comsol_data(:, 4);
scaling= sqrt(250/350);
emag = scaling.*comsol_data(:, 5);

% Initialize electron temperature (te)
te = 10 * ones(size(ne));

% Compute radial coordinate rC
rC = sqrt(xx.^2 + yy.^2);

% Visualize ne vs. rC and zz
figure; 
imagesc(rC, zz, ne);
colorbar;
title('Electron Density (ne)');
xlabel('Radial Coordinate (rC)');
ylabel('Z Coordinate (zz)');

% Scatter plot of 3D data
figure; 
scatter3(xx, yy, zz, [], emag, 'filled');
colorbar;
title('3D Scatter Plot of Emag');
xlabel('X Coordinate');
ylabel('Y Coordinate');
zlabel('Z Coordinate');
...
% te_surf=temp_value';
% ti_surf=te_surf;
% vp_surf= 0.25*sqrt(1.6E-19*te_surf./(16*MI));
% flux_surf = ne_surf.*vp_surf;
% 
% % ne_surf=ne;
% % te_surf=te;
% % ti_surf=te_surf;
...
% Define centroids array (ensure centroids array exists)
% Example:
% centroid = [1000, 2000, 300; 4000, 5000, -600; 7000, -8000, 900]; % Placeholder

% Initialize outputs
e_value = zeros(size(centroid, 1), 1);
dens_value = zeros(size(centroid, 1), 1);
temp_value = zeros(size(centroid, 1), 1);

% Process each centroid
for i = 1:length(centroid)
    % Compute distance from centroid to each COMSOL data point
...
MI = 1.67e-27; % Ion mass (kg, e.g., Hydrogen)
ne_surf = dens_value;
te_surf = temp_value;
ti_surf = te_surf; % Assuming ion temperature equals electron temperature
vp_surf = 0.25 * sqrt(1.6E-19 * te_surf ./ (16 * MI)); % Plasma velocity
flux_surf = ne_surf .* vp_surf; % Particle flux

% Output results for inspection
disp('Surface properties:');
disp(table(ne_surf', te_surf', vp_surf', flux_surf', 'VariableNames', ...
    {'Density', 'Temperature', 'PlasmaVelocity', 'Flux'}));

potential=e_value;
% Write surface data
surface_variables=table(potential,ne_surf,te_surf,ti_surf,vp_surf,b_mag,br_surf,bt_surf,bz_surf,theta);
...
figure;
patch(transpose(X),transpose(Y),transpose(Z),dens_value(plotSet)','FaceAlpha',1,'EdgeAlpha', 0.3 );
...
figure;
patch(transpose(X),transpose(Y),transpose(Z),temp_value(plotSet)','FaceAlpha',1,'EdgeAlpha', 0.3 );
title('ITER Limiter surface density')        
xlabel('x [mm]')
ylabel('y [mm]')
zlabel('z [mm]')
figure; 
patch(transpose(X),transpose(Y),transpose(Z),e_value(plotSet),'FaceAlpha',1,'EdgeAlpha', 0.3 )
...
te_data=data(:,3);
v_data=data(:,5);
erosion_flux=0.01.*ne_data.*v_data;
