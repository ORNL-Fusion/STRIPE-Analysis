...
% COMSOL data
comsol_data=readmatrix('5d4_VDC4.txt');
xx=comsol_data(:,1);
yy=comsol_data(:,2);
zz=comsol_data(:,3);
emag=comsol_data(:,4);
...
% Write surface data
surface_variables=table(potential,ne,te,ti,vp,b_mag,br_surf,bt_surf,bz_surf,theta, centroid);
...
te_data=data(:,3);
v_data=data(:,5);
erosion_flux=[yields_data(:,6)].*ne_data.*v_data;
