...
yields_data=readmatrix('yields_c1.csv');
potential_data=data(:,1);
ne_data=data(:,2);
ni_data=data(:,4);
te_data=data(:,3);
v_data=data(:,6);
ero_data=[yields_data(:,1)].*ni_data.*v_data;
theta=data(:,11);
...
Z = [transpose(z1(subset)),transpose(z2(subset)),transpose(z3(subset))];
%patch(transpose(X(surface,:)),transpose(Y(surface,:)),transpose(Z(surface,:)),impacts(surface),'FaceAlpha',.3)
patch(transpose(X),transpose(Y),transpose(Z),57.296*theta,'FaceAlpha',1,'EdgeAlpha', 0.3)%,impacts(surface)
title('Flux')
xlabel('X [m]')
ylabel('Y [m]')
zlabel('Z [m]')
