...
xlabel('\phi [rad]'); ylabel('\theta [rad]'); view(2);
 xlim([-0.484 0.484]);
    ylim([-0.7032 -0.635])
     ...
     % Add lighting
lighting gouraud        % or 'phong' for smoother shading
camlight headlight      % light from camera view (also try 'left' or 'right')
material dull           % or 'shiny', 'metal' depending on the look
...
sgtitle('Case # 196154', 'FontSize', 16);
