...
import shutil
import numpy as np
target = np.loadtxt('Targets_c2.txt', dtype='float',skiprows=1,delimiter=',')
...
for i in range(0, 12682):
    for j in range(2,3):
        ...
        impacts.diiid_case_C(surface_potential = surface_potential, charge = j, Tee=te, Tii = te, n=ni,btot = btot,theta=theta, sheath_factor = 1.0)
