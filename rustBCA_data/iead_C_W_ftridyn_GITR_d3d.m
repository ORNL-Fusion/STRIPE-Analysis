close all
clear all
ME = 9.10938356e-31;
MI = 1.6737236e-27;
Q = 1.60217662e-19;
EPS0 = 8.854187e-12;

amu = 18;
Tarray = [5,6,7,8,9,10];
narray = [1e16, 1e17, 5e17, 1e18, 5e18, 1e19];
Varray = [30, 160, 290, 420, 550, 680, 810, 940, 1070, 1200];


% file = 'Ta2O5_results.nc';
file = 'ftridynSelf_new_higResol.nc';

ncid = netcdf.open(file,'NC_NOWRITE');
[dimname, nE] = netcdf.inqDim(ncid,0);
[dimname, nA] = netcdf.inqDim(ncid,1);
if strcmp(file,'ftridynBackground.nc')
    [dimname, nS] = netcdf.inqDim(ncid,2);
else
    nS = 1;
end
energy = ncread(file,'energy_eV');
angle = ncread(file,'A');
spyld = ncread(file,'spyld');
rfyld = ncread(file,'rfyld');
cosxDist = ncread(file,'cosXDist');
cosxDistRef = ncread(file,'cosXDistRef');
cosyDist = ncread(file,'cosYDist');
eDist = ncread(file,'energyDist');
eDistRef = ncread(file,'energyDistRef');
eDistEgrid = ncread(file,'eDistEgrid');
phiGrid = ncread(file,'phiGrid');
thetaGrid = ncread(file,'thetaGrid');

if( nS > 1)
    for i=1:nS
        this_spyld = spyld(:,:,i);
        figure(i)
        h = pcolor(energy,angle,log10(this_spyld))
        h.EdgeColor = 'none';
        colorbar
        set(gca, 'XScale', 'log')
    end
else
    this_spyld = spyld;
    figure(1)
    h = pcolor(energy,angle,this_spyld)
    h.EdgeColor = 'none';
    colorbar
    set(gca, 'XScale', 'log')
    set(gca, 'ColorScale', 'log')
    xlabel('Energy [eV]')
    ylabel('Angle [degrees]')
    
    title({'Sputtering Yield','C on C'})
    load('coolwarm.mat', 'coolwarm_rgb')
    colormap(coolwarm_rgb)
 
end
energy_dist = linspace(0,10000,10000);
angle_dist = linspace(0,90,90);

[ee aa] = meshgrid(energy_dist,angle_dist);
Y0=interpn(energy,angle,spyld',ee,aa);

figure;  hold on;
plot(energy, this_spyld(5,:))
plot(energy, this_spyld(10,:))
plot(energy, this_spyld(30,:))
plot(energy, this_spyld(50,:))
plot(energy, this_spyld(60,:))
plot(energy, this_spyld(70,:))
plot(energy, this_spyld(80,:))
plot(energy, this_spyld(85,:))
plot(energy, this_spyld(88,:))
figure;hold on;

plot(angle, this_spyld(:,10))
plot(angle, this_spyld(:,20))
plot(angle, this_spyld(:,50))
plot(angle, this_spyld(:,100))
% plot(angle, this_spyld(:,200))
figure;

% --- First subplot: Yield vs Angle ---
subplot(211)
hold on;
Y0(isnan(Y0)) = 0;

% Define jet colormap with as many colors as the number of curves
colors = jet(7);
plot(angle, movmean(Y0(:,20),1),  'Color', colors(1,:), 'LineWidth', 1.5)
plot(angle, movmean(Y0(:,70),1),  'Color', colors(2,:), 'LineWidth', 1.5)
plot(angle, movmean(Y0(:,100),1), 'Color', colors(3,:), 'LineWidth', 1.5)
plot(angle, movmean(Y0(:,500),1), 'Color', colors(4,:), 'LineWidth', 1.5)
plot(angle, movmean(Y0(:,1000),1),'Color', colors(5,:), 'LineWidth', 1.5)
plot(angle, movmean(Y0(:,5000),1),'Color', colors(6,:), 'LineWidth', 1.5)
plot(angle, movmean(Y0(:,10000),1),'Color', colors(7,:), 'LineWidth', 1.5)

xlabel('Incidence angle (degrees)')
ylabel('Sputtering yield (atoms/ion)')
title('Yield vs Angle for Different Energies')
legend('20 eV', '70 eV','100 eV','500 eV','1000 eV','5000 eV','10000 eV','Location','NorthWest')
grid on; box on;
colormap jet; colorbar;
hold off;

% --- Second subplot: Yield vs Energy ---
subplot(212)
hold on;

angles = [5, 10, 30, 50, 60, 70, 80, 85, 88];
colors2 = jet(numel(angles));

for i = 1:length(angles)
    plot(energy, movmean(this_spyld(angles(i),:),5), 'Color', colors2(i,:), 'LineWidth', 1.5)
end

xlabel('Ion energy (eV)')
ylabel('Sputtering yield (atoms/ion)')
title('Yield vs Energy for Different Angles')
legend('5°','10°','30°','50°','60°','70°','80°','85°','88°','Location','NorthWest')
grid on; box on;
colormap jet; colorbar;
hold off;

% A = readmatrix('Targets.txt','NumHeaderLines',1);
r_mean = readmatrix('Centroid.csv');
         ne = readmatrix('ne0.csv');
         te = readmatrix('Te0.csv');
          ti = readmatrix('Te0.csv');
          vp = 0.25*sqrt(1.6E-19*te./(18*MI));
          btot = readmatrix('b_mag.csv');
%          br = A(:,10);
%           bt = A(:,11);
%           bz = A(:,12);

angle_imp = readmatrix('theta.csv');

nLoc = 340;
nCharge = 8;
yields = zeros(nLoc,nCharge);
meanE = zeros(nLoc,nCharge);
meanA = zeros(nLoc,nCharge);

for i=236:236
for j=6:6
filename = strcat('surface_C',string(j),'_loc_',string(i-1),'.nc');
surfEDist = ncread(filename,'surfEDist');

eff_yield = surfEDist.*Y0;
eff_yield(isnan(eff_yield)) = 0;

figure
h = pcolor(linspace(0,90,90),linspace(0,1000,1000),surfEDist')
h.EdgeColor = 'none';

eff_yield = sum(eff_yield(:))./sum(surfEDist(:));
%             yield = mean(Y0);
            yields(i,j) = eff_yield;
            
%             meanE(i,j) = mean(E);
%             meanA(i,j) = mean(impact_angle);
        end
end

return
%%
yields(isnan(yields))=0;


writematrix(yields,"yields.csv");
figure
plot(yields(:,5),'lineWidth',2)
% title({'ne'})
xlabel('location #') % x-axis label
ylabel('Yield') % y-axis label
set(gca,'fontsize',16)

Dflux = vp.*ne.*tand(angle_imp);
Dflux(isnan(Dflux))=0;
writematrix(Dflux,'Dflux.csv')
figure
semilogy(vp.*ne,'lineWidth',2)
hold on
semilogy(Dflux,'lineWidth',2)
C4_flux= 0.05*Dflux;
semilogy(C4_flux,'lineWidth',2)
% title({'ne'})
xlabel('location #') % x-axis label
ylabel('Flux [m-2s-1]') % y-axis label
set(gca,'fontsize',16)
legend('Parallel Flux','W','O')
eroded_flux=yields(:,5).*C4_flux(1:end);
writematrix(eroded_flux,'eroded_flux.csv')
figure
plot(yields(:,5).*C4_flux(1:end),'lineWidth',2)
% title({'ne'})
xlabel('location #') % x-axis label
ylabel('Eroded W Flux [m-2s-1]') % y-axis label
set(gca,'fontsize',16)

figure(5)
h = pcolor(narray,Tarray,yields(:,:,8)')
h.EdgeColor = 'none';
colorbar
set(gca, 'XScale', 'log')
%     set(gca, 'ColorScale', 'log')
xlabel('Density [m^{-3}]')
ylabel('Temperature [eV]')

title({'Sputtering Yield','O on Al 940 Volts'})
set(gca,'TickDir','out');
set(gca,'fontsize',14)
ax = gca;

k = 0.02
ax.TickLength = [k, k]; % Make tick marks longer.
ax.LineWidth = 10*k;

figure(6)
hold on
% for i=1:length(narray)
%     for j=1:length(Tarray)
        
        plot(Varray,reshape(meanE(3,3,:),1,[]),'Linewidth',2)
%     end
%     
% end
title({'Mean Impact Energy As Function of Surface Potential','n=10^{18} m^{-3} T = 7 eV'})
xlabel('Surface Potential [V]')
ylabel('Energy [eV]')
set(gca,'FontSize',14)
figure(7)
hold on
% for i=1:length(narray)
%     for j=1:length(Tarray)
        
        plot(Varray,reshape(meanA(3,3,:),1,[]),'Linewidth',2)
%     end
%     
% end
title({'Mean Impact Angle As Function of Surface Potential','n=10^{18} m^{-3} T = 7 eV'})
xlabel('Surface Potential [V]')
ylabel('Angle [degrees]')
set(gca,'FontSize',14)
% impact_angle = acosd(abs(vz)./sqrt(vx.^2 + vy.^2 + vz.^2));
% E = 0.5*amu*1.66e-27*(v(hit,1).^2+v(hit,2).^2+v(hit,3).^2)/Q;
% figure(120)
% hist3([90-impact_angle,E] ,'Edges',{0:90/180:90',0:1:2000'},'EdgeColor','none')
% set(get(gca,'child'),'FaceColor','interp','CDataMode','auto');
% colorbar
% title({'Energy Angle Distribution of All Surfaces [Counts]','5 degrees'})
% ylabel('Energy [eV]')
% xlabel('Angle [degrees]')
% set(gca,'FontSize',10)
% axis([0 90 0 1000])
% view(2)
save('yields_ntv_OAl.mat','narray','Tarray','Varray','yields')