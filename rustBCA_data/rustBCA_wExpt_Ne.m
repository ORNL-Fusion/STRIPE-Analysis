%% Ne_on_W_full_pipeline.m  (REVISED ONLY WHERE NEEDED)
...
%% ------------------------ User inputs ------------------------
file = 'ftridyn_NeonW_80keV.nc';         % RustBCA NetCDF file for Ne on W
out_csv = 'Ne_on_W_EXPT0_plus_EcksteinAngleFit.csv';
...
energy_reltol  = 0.06;        % used if you later cluster energies (not required here)
...
%% ------------------------ Experimental Ne+ -> W (0°) - IPP 9/82 ------------------------
Eexp_keV = [0.15, 0.30, 0.60, 1.00, 3.00, 10.00];  % keV
Eexp = 1e3 * Eexp_keV;                             % eV
Yexp = [0.086, 0.213, 0.272, 0.462, 0.747, 0.913]; % atoms/ion

% filter to available Euse range
okE = (Eexp >= Emin*0.999) & (Eexp <= Emax*1.001);
...
title('Ne^+ on W: Sputtering yield map (RustBCA)');
colorbar; grid on; box on;


% 2) Angle dependence (sim) at few energies (ANGLE AXIS IS LINEAR)
figure('Color','w'); hold on;
Elist = unique( max(min([50 100 200 500 1000 5000 10000], Emax), Emin) );
...
% ---- Key fix: if 0° requested but not present, plot A0 instead (still label as 0°) ----
...
title('Ne on W: Yield (RustBCA)');
legend([compose('%g°', A_list_plot) "EXPT 0°"], 'Location','northwest');
grid on; box on;
ylim([1e-6 1e1])
...
set(gca,'XScale','log'); set(gca,'YScale','log');
xlabel('Ion energy (eV)'); ylabel('Yield (atoms/ion)');
title(sprintf('Ne^+ on W: RustBCA(A0=%.2f°) vs EXPT 0° (and modeled angular points)', A0));
legend(sprintf('RustBCA %.2f° (effective 0°)',A0),'EXPT 0°','Modeled angle points','Location','northwest');
grid on; box on;

% 5) Quick diagnostic: fit vs RustBCA ratio at representative energy
