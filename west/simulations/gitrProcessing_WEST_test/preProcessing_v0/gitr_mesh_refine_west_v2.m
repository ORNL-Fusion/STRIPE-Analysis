...
planes = planes_from_stl('limiters_west_comsol.stl',0.03);
planes_copy=planes;
...
switch sheathType
    ...
    case 2 % RF rectified sheath (PISCES-RF)
        comsol_data=readmatrix('WEST0_VDC.csv');
        xx=comsol_data(:,1);
        zz=comsol_data(:,2);
        yy=comsol_data(:,3);
        ...
        % % ne_surf=double(interp1(double(z),double(ne(2414,:)),zz));
        % % te_surf=double(interp1(double(z),double(te(2414,:)),zz));
        % % 
        ...
        %     % 
        %     % if abs(centroid(i,3))> 1.1578+0.1878 % ICH: 3.5+0.2 %
        %     %     potential_surf(i) = 0;
        %     %     dens_value(i) = 0;
        %     %     temp_value(i) = 0;
        %     % end
...
% Write surface data
surface_variables=table(potential_surf',ne_surf,te_surf,te_surf,vp_surf,b_mag,br_surf,bt_surf,bz_surf,theta);
