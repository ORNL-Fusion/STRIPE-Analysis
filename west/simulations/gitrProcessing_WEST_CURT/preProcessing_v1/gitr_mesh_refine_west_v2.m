...
% planes = planes_from_stl('west_limiters_4cm.stl',0.0766); %COMSOL
% planes = planes_from_stl('west_limiters_4cm.stl',0.04); %COMSOL TEST
% planes = planes_from_stl('west_limiters_4cm.stl',0.075); %Thermal
% planes = planes_from_stl('west.stl',0.008);
planes = planes_from_stl('simplified_west_lower_target.stl',0.01); %DIVERTOR
...
y_faces = find(abs(centroid(:,3)-0.005) <= 0.005 );
...
% HeliconRefined=find(abs(centroid(:,3)-0.16) <= 2.94 &(centroid(:,1)<=0.3583) & (centroid(:,1)>=-0.3583)); % WESR ICT antenna
HeliconRefined=find(abs(centroid(:,3)-0.005) <= 0.005 & (centroid(:,1)<=0.3583) & (centroid(:,1)>=-0.3583));
...
figure
patch(transpose(X),transpose(Y),transpose(Z),[.7 .7 .7],'FaceAlpha',.3,'EdgeColor','none')%'none')
title({'GITR Geometry'})
xlabel('X [m]')
ylabel('Y [m]')
zlabel('Z [m]')
    ...
    case 1 % Thermal sheath
     ...
     case 2 % RF rectified sheath (COMSOL)
        ...
        comsol_data=readmatrix('comsol_data/VDC_out_WEST57877_it3.txt');
         comsol_data=readmatrix('VDC_out_halfPower_it1.txt');
        xx=comsol_data(:,1);
        zz=comsol_data(:,2);
        yy=comsol_data(:,3);
...
surface_variables=table(potential_surf,ne_surf,te_surf,ti_surf,vto_surf,b_mag,br_surf,bt_surf,bz_surf,theta,ni_surf);
