...
vt0 = zeros(size(viz_q));
vz0 = zeros(size(viz_q));
vr0 = zeros(size(viz_q));
...
bz0=zeros(size(bz_q)      );
bt0=zeros(size(bphi_q)    );
vt0 = zeros(size(vzo_q{charge}));
vz0 = zeros(size(vzo_q{charge}));
vr0 = zeros(size(vzo_q{charge}));

ti0 = zeros(size(to_q{charge}) );
te0 = zeros(size(te_q)    );

ne0 = zeros(size(ne_q)    );
ni0=zeros(size(no_q{charge})   );
%}
...
% ti0 = double(to_q{charge}); % Oxygen
ti0 = double(ti_q); % D+
te0 = double(te_q);
ne0 = double(ne_q);
% ne0(find(x0>=rW & z0>=zW))=NaN;
% ni0=double(no_q{charge}); % Oxygen
ni0=double(ni_q); % Deuterium
% v0 = double(vzo_q{charge})./ni0; % Oxygen
% % v0 = double(vzo_q{charge}); % Oxygen
v0 = double(viz_q); % D+
v0(find(rq>=rW & zq>=zW))=0;
vz0 = v0.*ubz;
vr0 = v0.*ubr;
vt0 = v0.*ubt;
