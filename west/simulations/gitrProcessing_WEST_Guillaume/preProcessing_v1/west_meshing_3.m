...
%% Load STL Geometry (Geometry 1 - Wall)
model = createpde;
...
%% Load antenna geometry from .mat file
load('west_antenna_geom.mat');  % Assumes variables 'planes' and 'potential' exist

% Combine STL + Antenna geometry
planes = [planes1; planes];  % Combined geometry

toc

% Identify which triangles came from antenna
...
%% Apply potential from .mat file
disp('>>>> Applying surface potentials from .mat antenna geometry')

% Apply potential only to triangles from .mat geometry
for i = inds_mat
    distance = sqrt((centroid(i,1) - xx).^2 + (centroid(i,2) - yy).^2 + (centroid(i,3) - zz).^2);
    [~, I] = min(distance);
    % if centroid(i,3) > -0.4198 && centroid(i,3) < 1.6599
        potential(i) = potential_surf(I);
    % else
    %     potential = 0;
    % end
end
...
figure;
patch(transpose(X), transpose(Y), transpose(Z), potential_surf, ...
      'FaceAlpha', 0.1, 'EdgeColor', 'none');
title('Potential applied only on Antenna geometry')
xlabel('X'); ylabel('Y'); zlabel('Z');

figure;
scatter3(centroid(:,1),centroid(:,2),centroid(:,3), 20, potential_surf, 'filled');

%% Write GITR geometry config
disp('>>>> Generating geometry file for GITR')
...
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,planes(i,1)); end

fprintf(fileID,' ] \n   y1 = [');
fprintf(fileID,format,planes(1,2));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,planes(i,2)); end

fprintf(fileID,' ] \n   z1 = [');
fprintf(fileID,format,planes(1,3));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,planes(i,3)); end

fprintf(fileID,' ] \n   x2 = [');
fprintf(fileID,format,planes(1,4));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,planes(i,4)); end

fprintf(fileID,' ] \n   y2 = [');
fprintf(fileID,format,planes(1,5));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,planes(i,5)); end

fprintf(fileID,' ] \n   z2 = [');
fprintf(fileID,format,planes(1,6));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,planes(i,6)); end

fprintf(fileID,' ] \n   x3 = [');
fprintf(fileID,format,planes(1,7));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,planes(i,7)); end

fprintf(fileID,' ] \n   y3 = [');
fprintf(fileID,format,planes(1,8));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,planes(i,8)); end

fprintf(fileID,' ] \n   z3 = [');
fprintf(fileID,format,planes(1,9));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,planes(i,9)); end

fprintf(fileID,' ] \n   a = [');
fprintf(fileID,format,abcd(1,1));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,abcd(i,1)); end

fprintf(fileID,' ] \n   b = [');
fprintf(fileID,format,abcd(1,2));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,abcd(i,2)); end

fprintf(fileID,' ] \n   c = [');
fprintf(fileID,format,abcd(1,3));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,abcd(i,3)); end

fprintf(fileID,' ] \n   d = [');
fprintf(fileID,format,abcd(1,4));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,abcd(i,4)); end

fprintf(fileID,' ] \n   plane_norm = [');
fprintf(fileID,format,plane_norm(1));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,plane_norm(i)); end

fprintf(fileID,' ] \n   BCxBA = [');
fprintf(fileID,format,BCxBA(1));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,BCxBA(i)); end

fprintf(fileID,' ] \n   CAxCB = [');
fprintf(fileID,format,CAxCB(1));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,CAxCB(i)); end

fprintf(fileID,' ] \n   area = [');
fprintf(fileID,format,area(1));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,format,area(i)); end

fprintf(fileID,' ] \n   Z = [');
fprintf(fileID,'%f',materialZ(1));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,'%f',materialZ(i)); end

fprintf(fileID,' ] \n   surface = [');
fprintf(fileID,'%i',surfs(1));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,'%i',surfs(i)); end

fprintf(fileID,' ] \n   inDir = [');
fprintf(fileID,'%i',inDir(1));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,'%i',inDir(i)); end

fprintf(fileID,' ] \n   potential = [');
fprintf(fileID,'%f',potential(1));
for i=2:nFaces, fprintf(fileID, ','); fprintf(fileID,'%f',potential(i)); end
