...
export GLI_HOME=/home/cloud/myRepos/S3XE_new/styx-eirene/external_libs/gks
...
# Executable
EXECDIR=/home/cloud/myRepos/S3XE_new/build # !!! CHANGE HERE!!!
EXECNAME=soledge3x-eirene # !!! CHANGE HERE!!!
...
MESHNAME=mesh_S3X # !!! CHANGE HERE!!! (if necessary)

# Database directories
S3XDIR=/home/cloud/myRepos/S3XE_new/soledge3x # !!! CHANGE HERE!!!
STYXDIR=/home/cloud/myRepos/S3XE_new/styx  # !!! CHANGE HERE!!!
...
#cp $CASEDIR/diffusion.h5 $RUNDIR/diffusion.h5
cp $PARAMDIR/param_raptorX.txt $RUNDIR/.
#cp $CASEDIR/param_geom.txt $RUNDIR/.
#cp $CASEDIR/source_raptorX.txt $RUNDIR/.

# SOLEDGE3X database for AM rates between ionized species
cd $RUNDIR
mkdir -v AMSplines
rsync -tv $DBDIR_S3X/* ./AMSplines/.
...
# Launch the job
################
#time mpirun --mpi=pmi2 -K1 $SRUN_CPU_BIND_OPTIONS $EXECDIR/$EXECNAME $PETSC_OPTIONS
time mpirun -np 64  $EXECDIR/$EXECNAME $PETSC_OPTIONS
