...
%% End cap physics
nExited = sum(weight0(find(z0>= 0.3599 & z0<=0.4201)));
...
for i=1:nBins
    ...
    hist_bins(i) = sum(weight0(find(z0>= 0.3599 & z0<=0.4201 & sqrt(x0.^2+y0.^2) > edges(i) & sqrt(x0.^2+y0.^2) <= edges(i+1))));
...
%For ionized
hitCap1 = find(z0>= 0.3599 & z0<=0.4201 & charge0>0);
...
cap1 = find(Zsurface ==0 & z1 < 0.42001);
...
RadialSlice1=slice1(:,:,1384); % Helicon Antenna
%RadialSlice1=sum(slice1(:,:,5875:5905)); %5905
% RadialSlice1=sum(slice1(:,:,1315:1384)); %5905
% RadialSlice1=reshape(RadialSlice1,1384);
slice1 = sum(slice1,2);
...
AverageRadialDensity1=movmean(RadialDensity1(35,:),5);
...
title('Al ions Impurity Density at Z=3.5 m')
xlim([-0.06 0.06])
ylim([-0.06 0.06])
xlabel('X [cm]')
ylabel('Y [cm]')
