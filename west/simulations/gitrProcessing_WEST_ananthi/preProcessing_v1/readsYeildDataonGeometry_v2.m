...
yields_data = readmatrix('yields_low_b1+.csv');
...
ero_data = [0;0;yields_data(:,1)] .* ne_data .* v_data;
sputtering_yield = [0;0;yields_data(:,1)];
...
% Subplot 3: Gross Erosion Flux/D-flux
figure; 
patch(transpose(phi), transpose(theta), 0 .* transpose(theta), te_data, ...
    'FaceAlpha', 1, 'EdgeAlpha', 0.3);
title('Gross Erosion Flux/D-flux', 'FontSize', 18);
colorbar('eastoutside');
xlabel('Phi [rad]', 'FontSize', 18);
