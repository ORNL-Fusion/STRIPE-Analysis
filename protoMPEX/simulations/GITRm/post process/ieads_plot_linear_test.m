%% revised_script_physical_eV_angle_with_true_combined_thresholded.m
...
%% ----------------------------
tilt = 85;
file = sprintf('../tilted_targets/test/%d_degrees/MPEX_runs_wCol/gitrm-surface.nc', tilt);
...
% % To include neutrals, use:
% BIN_TA = 2:6;
% ta_labels = {'Ta^0','Ta1^+','Ta2^+','Ta3^+','Ta4^+'};
...
% Low-count filtering for the final combined plot only
APPLY_LOW_COUNT_FILTER = true;
LOW_COUNT_PERCENTILE = 10;   % Try 5, 10, or 15
MIN_ABSOLUTE_COUNT = 0;      % Set to 0.05 if desired
...
%% ----------------------------
tmp_comb = sum(surfEDist_all(:,:,:,:,BIN_TA),5);
AES_comb = squeeze(sum(tmp_comb,4));
...
%% ----------------------------
% SMOOTHED + LOW-COUNT FILTERED COMBINED IEAD
%% ----------------------------
Z = AE_comb(:,Eidx);
E = energy_eV(Eidx);
A = angle_deg;
...
if APPLY_LOW_COUNT_FILTER
    finiteVals = Zplot(isfinite(Zplot) & Zplot > 0);

    if ~isempty(finiteVals)
        percentile_threshold = prctile(finiteVals,LOW_COUNT_PERCENTILE);
        count_threshold = max(percentile_threshold,MIN_ABSOLUTE_COUNT);

        fprintf('Low-count threshold applied to combined IEAD = %.4g\n', count_threshold);

        Zplot(Zplot < count_threshold) = NaN;
    end
end
...
title(sprintf('Filtered Combined Incident IEAD, tilt = %d^\\circ',tilt), ...
    'FontSize',18,'FontWeight','bold')

xlim([0 100])
ylim([0 90])

finiteVals = Zplot(isfinite(Zplot));
if ~isempty(finiteVals)
    caxis([0 prctile(finiteVals,99.5)])
end
