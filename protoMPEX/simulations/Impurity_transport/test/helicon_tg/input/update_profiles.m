...
file = 'profilesProtoMPEX_solps.nc';
...
ne_interp=interpn(x0,z0,ne0,xx,zz);
ne_interp(:,1)=ne_interp(:,2);
ni_interp=ne_interp;
te_interp=te1;
% te_interp(:,1)=te_interp(:,2);
ti_interp=ti1;
% ti_interp(:,1)=ti_interp(:,2);



vz_interp=interpn(x0,z0,vz0,xx,zz);
MachNum_interp=interpn(x0,z0,MachNum,xx,zz);
MachNum_interp(:,1)=MachNum_interp(:,3);
MachNum_interp(:,2)=MachNum_interp(:,3);

figure;imagesc(z1,x1,MachNum_interp');
figure;imagesc(z1,x1,te_interp)

figure; plot(z1,MachNum_interp(:,1))
figure; plot(z1,ne_interp(:,1))



vr_interp=interpn(x0,z0,vr0,xx,zz);
vt_interp=interpn(x0,z0,vt0,xx,zz);
gradTi_interp=interpn(x0,z0,gradTi0,xx,zz);

ne_interp(isnan(ne_interp))=0;
vt_interp(isnan(vt_interp))=0;
vr_interp(isnan(vr_interp))=0;
vz_interp(isnan(vz_interp))=0;
gradTi_interp(isnan(gradTi_interp))=0;


figure;pcolor(z1(1:25:end-1),x1(1:25:end-1),ne_interp(1:25:end-1,1:25:end-1)');




vz1 = MachNum_interp'.*(sqrt((8/(2*pi))*k_B.*11604.*te_interp./(2*m_p)));
% % ne0 = ne;
% % ni0=ne;
% vz0=vz1;
...
netcdf.endDef(ncid);
% 
netcdf.putVar(ncid,gridRnc,x1);
netcdf.putVar(ncid,gridZnc,z1);
netcdf.putVar(ncid,Ne2Dnc,ne_interp');
netcdf.putVar(ncid,Ni2Dnc,ni_interp');
netcdf.putVar(ncid,Te2Dnc,te_interp);
netcdf.putVar(ncid,Ti2Dnc,ti_interp);
netcdf.putVar(ncid,gradTi2Dnc,gradTi_interp');

netcdf.putVar(ncid,vrnc,vr_interp');
netcdf.putVar(ncid,vtnc,vt_interp');
netcdf.putVar(ncid,vznc,vz1);
