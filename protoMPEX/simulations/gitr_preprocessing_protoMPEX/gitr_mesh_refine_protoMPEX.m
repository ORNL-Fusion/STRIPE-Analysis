...
planes = planes_from_stl('../CAD/helicon_cylinder_4p14m.stl',0.024);
refinementnum=4;
toc
...
r_centroid1=sqrt(centroid(:,1).^2+centroid(:,2).^2);
z_centroid1=centroid(:,3);
...
% protoMPEX_profile
protoMPEX_profile_solps
protoMPEX_helicon_data


% [xx yy zz emag ne_surf_josh] = helicon_read_xls(surface_file);

% zz=zz+1.7462;% helicon

% ne_surf=interp2(x0,z0,ne0(2500,:),xx,zz);
% zz=zz+3.5;% ICH




comsol_data=readmatrix('comsol/ec0kw_VDC.txt');
% comsol_data=readmatrix('comsol/ec4kw_VDC.txt');
% comsol_data=readmatrix('../comsol/ec30kw_VDC.txt');
xx=comsol_data(:,1);
yy=comsol_data(:,2);
zz=comsol_data(:,3);
emag=comsol_data(:,4);
emag(find(emag>1200))=1200;


zz=zz+1.7462;
ne_surf=double(interp1(double(z),double(ne(2500,:)),zz));
te_surf=double(interp1(double(z),double(te(2500,:)),zz));
% figure; plot(zz,ne_surf_josh,'o',zz,ne_surf,':.')
figure; plot(zz,ne_surf,':.')
figure; plot(zz,te_surf,':.')
...
for i=1:length(centroid)
    ...
    if abs(centroid(i,3))> 1.7462+0.18 % ICH: 3.5+0.2 %
        e_value(i) = 0;
        dens_value(i) = 0;
         % dens_value_josh(i) = 0;
        temp_value(i) = 0;
...
patch(transpose(Z),(transpose(THETA)+3).*57.2958,0*transpose(Z),e_value(plotSet),'FaceAlpha',1,'EdgeColor','k')
hold on
scatter(Mask(:,1),Mask(:,2),pointsize,(max(e_value)*Mask(:,3)),'MarkerFaceAlpha',0.4,'MarkerEdgeAlpha',0.1);
colorbar
title({'Surface Potential [V]'})
...
Y0 = interpn(narray,Tarray,Varray,yields,ion_dens_wall,0.*ion_dens_wall+8,e_value,'pchip',0);%SOLPS-ITER case
% Y0(isnan(Y0))=0;
% Y0(find(Y0==0 ))=0.02;
% Y0(find(abs(zz-0.2)>=1.7462))=0.0;
...
% ion_dens_wall = interp2(z, x, ne,  1.745, 0.0624);
% ion_temp_wall = interp2(z, x, te,  1.745, 0.0624);
...
% hold on;
% scatter(Mask(:,1),Mask(:,2),pointsize,(max(flux(plotSet).*Y0(plotSet)).*Mask(:,3)),'MarkerFaceAlpha',0.4,'MarkerEdgeAlpha',0.1);
...
figure
%patch(transpose(THETA),transpose(Z),0*transpose(Z),Y0(plotSet),'FaceAlpha',1,'EdgeColor','k')
patch(transpose(X),transpose(Y),transpose(Z),flux(plotSet).*Y0(plotSet),'FaceAlpha',1,'EdgeColor','k')
% hold on;
% scatter(Mask(:,1),Mask(:,2),pointsize,(max(flux(plotSet).*Y0(plotSet)).*Mask(:,3)),'MarkerFaceAlpha',0.4,'MarkerEdgeAlpha',0.1);
...
% erosion(find(centroid(:,3)>1.8))=0;

figure;
patch(transpose(((THETA))),transpose(Z),0*transpose(Z),erosion(plotSet),'FaceAlpha',1,'EdgeColor','k')
...
% figure;
% patch(transpose(((THETA(erosion_inds,:)))),transpose(Z(erosion_inds,:)),0*transpose(Z (erosion_inds,:)),erosion_sub,'FaceAlpha',1,'EdgeColor','k')



figure; plot(erosion_sub)
...
% scatter(theta_sample,z_sample,0*z_sample,'r')
% scatter(theta_sample,z_sample,'r')
