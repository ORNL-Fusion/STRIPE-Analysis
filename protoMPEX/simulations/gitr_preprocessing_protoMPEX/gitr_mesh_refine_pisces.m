...
planes = planes_from_stl('../CAD/pisces_rf_new.stl',0.024);
refinementnum=4;
toc
...
helcenter=1.1578;% helicon
% helcenter=3.5; % ICH

helregion1=helcenter-0.1878;
Upstream=0.6436;
helregion2=helcenter+0.1878;
Downstream=3.09;
...
% piscesRF_profile
% piscesRF_helicon_data
% 
% piscesRF_profile_solps
piscesRF_profile
piscesRF_helicon_data_slops
% 

% [xx yy zz emag ne_surf_josh] = helicon_read_xls(surface_file);
% comsol_data=readmatrix('1e16_1e18_VDC_10kw.txt');
comsol_data=readmatrix('../comsol/1_1_VDC_case1.txt');
comsol_data=readmatrix('../comsol/1_1_VDC_case2.txt');
comsol_data=readmatrix('../comsol/1_1_VDC_case3.txt');
comsol_data=readmatrix('../comsol/1_1_VDC_case4.txt');
comsol_data=readmatrix('../comsol/1_1_VDC_case5.txt');
comsol_data=readmatrix('../comsol/1_1_VDC_case6.txt');
...
% 
% ne_surf=interp2(x0,z0,ne0(2414,:),xx,zz);
% zz=zz+3.5;% ICH
...
for i=1:length(centroid)
    ...
    if abs(centroid(i,3))< 0.97 % ICH: 3.5+0.2 %
    % if abs(centroid(i,3)-0.1878)> 1.1578 % ICH: 3.5+0.2 %
        e_value(i) = 0;
        dens_value(i) = 0;
        %  dens_value_josh(i) = 0;
        temp_value(i) = 0;
...
% dens_value=double(interp1(z0,ne0(2414,:),zz));
% temp_value=double(interp1(z0,te0(2414,:),zz));
...
%%
figure(103)
patch(transpose(Z),transpose((180-(THETA))+180),0*transpose(Z),(flux(plotSet).*Y0(plotSet)),'FaceAlpha',1,'EdgeColor','k')
...
figure
patch(transpose(Z),(transpose(THETA)),0*transpose(Z),(flux(plotSet).*Y0(plotSet))','FaceAlpha',1,'EdgeColor','k')
% patch(transpose(X),transpose(Y),transpose(Z),flux(plotSet).*Y0(plotSet),'FaceAlpha',1,'EdgeColor','k')
