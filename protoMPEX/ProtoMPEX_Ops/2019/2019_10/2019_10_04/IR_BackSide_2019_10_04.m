...
a{2} = [RootAddress,'\2019_10_04'];

% note:
% in shot 28071, we can see that the raw pixel intensities from the IR
% camera lead to an almost linear regime in the stefan boltzmann relation
...
for s = 1:size(RawData,1)
    ...
    for ii = 1:size(RawData{1},3)
        IntensityIntegrated(ii) = sum(sum(RawData{s}(:,:,ii)))/N;
    end
end

figure; 
plot(IntensityIntegrated)
yyaxis right
plot(diff(IntensityIntegrated),'r')
