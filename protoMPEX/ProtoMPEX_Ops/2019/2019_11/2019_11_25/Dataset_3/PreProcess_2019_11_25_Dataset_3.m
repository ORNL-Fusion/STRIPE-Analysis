...
if cmptThresholds
   save('preprocess_Dataset_3','mpg_ginput','windowArea','-append');
   save('preprocess_Dataset_3','t_intensity','intensity','-append');
   save('preprocess_Dataset_3','shot','scanVar','addressLoc','-append'); 
   save('preprocess_Dataset_3','viewType','-append'); 
   save('preprocess_Dataset_3','seq','-append'); 
end
...
for fr = 1:1:size(intensity{s},3)
    ...
    view([0,90])
    zlim([0,intensity0_mpp(s)*7])
    caxis([0,intensity0_mpp(s)*7])
    colormap(flipud(hot))
    title(['shot: ',num2str(shot(s)),', frame: ',num2str(fr),' ',viewType{xp}])
    axis('equal')
    xlim([0,size(intensity0_noOffset{s},2)])
    ylim([0,size(intensity0_noOffset{s},1)])
    hold on
