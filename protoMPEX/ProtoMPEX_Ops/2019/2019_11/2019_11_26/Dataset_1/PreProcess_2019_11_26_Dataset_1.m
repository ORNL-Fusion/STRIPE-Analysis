...
switch dataset
    case 1
        ...
        scanVar    =         [1      ,2      ];
        addressLoc =         [1      ,1      ];
        viewType   =         {'M-Btm','W-Btm'}; 
        fileName = ['preprocess_Dataset_1'];
    case 2
        shot       = 28000 + [421    ,416    ];
        scanVar    =         [1      ,2      ];
        addressLoc =         [1      ,1      ];
        viewType   =         {'M-Mdl','W-Mdl'}; 
        fileName = ['preprocess_Dataset_2'];
    case 3
        shot       = 28000 + [420    ,417    ];
        scanVar    =         [1      ,2      ];
        addressLoc =         [1      ,1      ];
        viewType   =         {'M-Top','W-Top'}; 
        fileName = ['preprocess_Dataset_3'];
    case 4
        shot       = 28000 + [417    ,416    ,415    ];
        scanVar    =         [1      ,2      ,3      ];
        addressLoc =         [1      ,1      ,1      ];
        viewType   =         {'W-Top','W-Mdl','W-Btm'}; 
        fileName = ['preprocess_Dataset_4'];
    case 5
        shot       = 28000 + [420    ,421    ,422    ];
        scanVar    =         [1      ,2      ,3      ];
        addressLoc =         [1      ,1      ,1      ];
        viewType   =         {'M-Top','M-Mdl','M-Btm'}; 
        fileName = ['preprocess_Dataset_5'];
    case 6
        shot       = 28000 + [424        ,427        ];
        scanVar    =         [1          ,2          ];
        addressLoc =         [1          ,1          ];
        viewType   =         {'M-Btm-hgs','M-Btm-lgs'}; 
        fileName = ['preprocess_Dataset_6'];
end
...
% =========================================================================
a{1} = ['C:\Users\nfc\Documents\ProtoMPEX_Ops\2019\2019_11\2019_11_26\IR_Data'];
...
inputStructure.prompt = 'Compute thresholds? Yes [1], No [0]';
cmptThresholds = GetUserInput(inputStructure);
...
for s = 1:nshots
    ...
    axis('equal')
    set(gca,'XTick',[],'YTick',[])
    hTitle = title([num2str(shot(s)),' ',viewType{s}]);
    hTitle.FontSize = 8;
    clear hTitle
...
if cmptThresholds
   save(fileName,'mpg_ginput','windowArea','-append');
   save(fileName,'t_intensity','intensity','-append');
   save(fileName,'shot','scanVar','addressLoc','-append'); 
   save(fileName,'viewType','-append'); 
   save(fileName,'seq','-append'); 
end
...
nshot = length(shot);
for fr = 1:3:size(intensity{s},3)
    for s = 1:nshot
    subplot(nshot,1,s)
    intensity_windowArea = intensity{s}(:,:,fr).*windowArea{s}  -  intensity{s}(:,:,1).*windowArea{s};
    surf(intensity_windowArea,'LineStyle','none')
    view([0,90])
    zlim([0,intensity0_mpp(2)*6])
    caxis([0,intensity0_mpp(2)*6])
    colormap(flipud(hot))
    title(['shot: ',num2str(shot(s)),', frame: ',num2str(fr),' ',viewType{s}])
    axis('equal')
    xlim([0,size(intensity0_noOffset{s},2)])
    ylim([0,size(intensity0_noOffset{s},1)])
    hold on
