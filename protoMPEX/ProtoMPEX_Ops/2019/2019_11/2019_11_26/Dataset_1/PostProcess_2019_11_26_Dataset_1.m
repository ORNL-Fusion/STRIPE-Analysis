...
switch dataset
    case 1
        load(['preprocess_Dataset_1.mat'])
    case 2
        load(['preprocess_Dataset_2.mat'])
    case 3
        load(['preprocess_Dataset_3.mat'])
    case 4
        load(['preprocess_Dataset_4.mat'])
    case 5
        load(['preprocess_Dataset_5.mat'])
    case 6
        load(['preprocess_Dataset_6.mat'])    
end
...
for s = 1:length(shot)
    ...
    [X{s},Y{s}] = meshgrid(rngy_temperature_probe{s},rngx_temperature_probe{s});
    Z{s} = 16*ones(size(Y{s}));
...
for fr = 1:3:size(temperature{s},3)
    ...
    hold on
    plot3(X{s}(:,end),Y{s}(:,end),Z{s}(1,:),'k.','MarkerSize',0.5)
    plot3(X{s}(1,:  ),Y{s}(1,:  ),Z{s}(1,:),'k.','MarkerSize',0.5)
    plot3(X{s}(:,1  ),Y{s}(:,1  ),Z{s}(1,:),'k.','MarkerSize',0.5)
    plot3(X{s}(end,:  ),Y{s}(end,:  ),Z{s}(1,:),'k.','MarkerSize',0.5)
    hold off
    h_cb = colorbar;
    h_text = text(50,150,50,{viewType{s};['shot: ',num2str(shot(s))];num2str(scanVar(s))});
...
title(viewType{s},'Interpreter','latex','FontSize',11)
