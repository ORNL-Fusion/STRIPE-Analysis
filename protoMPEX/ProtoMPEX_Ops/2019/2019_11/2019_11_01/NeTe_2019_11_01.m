...
switch ShotType
    case 1
        shotlist = [28000 + [138,142,143]];
        shotlist = [28000 + [143,144,145,146]];
        shotlist = [28000 + [147,148,149,150]];
        X        =          [9.5,8.5,7.5,6.5];
        shotlist = [28000 + [155,156,157]];
        shotlist = [28000 + [157,158]];
        shotlist = [28000 + [161,163,164,165,166,167,168]];
        shotlist = [28000 + [164,169,171]];
        shotlist = [28000 + [172]];
        shotlist = [28000 + [174,175]];        
    case 2
         shotlist = [28000 + [145]];        
         shotlist = [28000 + [138]];        
...
for s = 1:length(shotlist)
%     GoodFits{s} = GlitchFlag{s} == 0 & StdResNorm{s}<=2 & Ni{s}>0 & Ni{s}<50e19 & Te{s}<=+30 & Is{s}>0.01e-3;
     GoodFits{s} = abs(Ni{s})>0; 
    t_Ni_gf{s} = time{s}(GoodFits{s} & abs(Ni{s})>1e16);
    Ni_gf{s} = Ni{s}(GoodFits{s} & abs(Ni{s})>1e16);
    
    h(s) = plot(t_Ni_gf{s},abs(Ni_gf{s}),C{s},'lineWidth',2);
