...
shotlist = [27838,28000 + [265]];
shotlist = [28000 + [145,266]];

shotlist = [28000 + [286,289]];
shotlist = [28289       28290       28291       28307];
...
for s = 1:length(shotlist)
    plot(t_f1{s}(1:end-1),(f1{s}-b*min(f1{s}))*20,C1{s},'LineWidth',1)
    plot(t_f2{s}(1:end-1),(f2{s}-b*min(f2{s}))*20,C2{s},'LineWidth',1) % Ground Side
    plot(t_f3{s}(1:end-1),(f3{s}-b*min(f3{s}))*20,C3{s},'LineWidth',1) % High Voltage side
    plot(t_f4{s}(1:end-1),(f4{s}-b*min(f4{s}))*20,C4{s},'LineWidth',1)
end
