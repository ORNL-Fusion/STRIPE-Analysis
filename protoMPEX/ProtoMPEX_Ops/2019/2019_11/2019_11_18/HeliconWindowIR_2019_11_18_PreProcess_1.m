...
switch xp
    case 1        
        shot       = 28000 + [289,290,291,292,293,305,307];
        rfPwrNet   =         [118,105,90 ,67 ,48 ,125,129];
        addressLoc =         [1  ,1  ,1  ,1  ,1  ,1  ,1  ];
        viewType   = 'Window_Limit_Bottom'; 
     
    case 2
        shot       = 28000 + [294,295,296,297,298,299,300,301];
        rfPwrNet   =         [125,108,91 ,55 ,75 ,132,138,146];
...
for fr = 1:1:size(intensity{s},3)
    ...
    view([0,90])
    zlim([0,intensity0_mpp(s)*2])
    caxis([0,intensity0_mpp(s)*2])
    colormap(flipud(hot))
