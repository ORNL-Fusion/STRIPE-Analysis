...
% If it exists, modify vars
% If it does not exist, create vars
vars.task3.section1 = who;
vars.task3.section1 = setdiff(vars.task3.section1,[]);
% Save it to "vars_list.mat')
try
    save('vars_list','vars','-append')
catch
    save('vars_list','vars')
end
% If we consider making a function that we call to produce a snapshot of
% the variables thur far we should consider the following inbuilt function
% evaluate from other workspaces if one uses functions
% vars = evalin('base','who');
% this function would work in pairs just like tic toc work and the purpose
% is record the names of all the variables created between two points
...
if cmpt == 0
    ...
    fileName = ['Task3_memory.mat'];
    load('vars_list.mat')
    variableNames = vars.task3.section2;
...
for s = 1:nshots
    subplot(nRow,nCol,s)
    hold on
    hDummy1(1) = plot(t_z{s}*t_star,heatflux_tempProbe{s}*1e-3             ,C{s},'LineWidth',2);
    hDummy1(2) = plot(t_z{s}*t_star,heatflux_tempProbe_infiniteSlab{s}*1e-3,C{s},'LineWidth',1);
    xlim([0,1])
    ylim([0,700])
    box on
    grid on
...
if cmpt
    ...
    % If it exists, modify vars
    % If it does not exist, create vars
    vars.task3.section2 = who;
    vars.task3.section2 = setdiff(vars.task3.section2,vars.task3.section1);
    % Save it to "varsList.mat')
    try
        save('vars_list','vars','-append')
    catch
        save('vars_list','vars')
...
for s = 1:nshots
    hDummy1(s) = plot(t_z{s}*t_star,heatflux_tempProbe{s}*1e-3,C{s},'LineWidth',2);
    xlim([0,1])
    ylim([0,800])
    box on
    grid on
...
ylabel('[MWm$^{-2}$]','Interpreter','latex','FontSize',12)
