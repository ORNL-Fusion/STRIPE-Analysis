...
% Compute prompt:
if defaultAns ~= 1
    InputStructure.prompt = {['Would you like to reload DLP data from MPEX server? Yes [1], No [0]']};
    beep
    cmpt = GetUserInput(InputStructure);
else
    cmpt = 1;
...
% #########################################################################
% SECTION 2:
sectionName = 'Load DLP data from MPEX server';
disp(['Section: ',sectionName])
% #########################################################################
...
if strcmp(datasetTable.scanType(shotIndex(1)),'RF power')
        figure('Tag','DLP vs RfPwr','color','w')
        plot(X,dataDLP_ss{1},'ko')
        xlim([0,200])
        ylim([0,1.5e20])
        ...
        if saveFig
            disp('Saving figures...')
            hdum = findobj('Tag','DLP vs RfPwr');
            figureName = ['dataset_',num2str(datasetToAnalyze),'_DLP_vs_rfPwr'];
            saveas(hdum,figureName,'tiffn')
            disp('Figure saved!')
        end
end
