...
a{2} = ['\\mpexserver\ProtoMPEX_Data\IR_Camera\2020_04_15'];
a{3} = ['\\mpexserver\ProtoMPEX_Data\IR_Camera\2020_03_31'];
a{4} = ['\\mpexserver\ProtoMPEX_Data\IR_Camera\2020_04_08'];
...
for s = 1:length(shot)
    ...
    % Rotate image:
    angle = -45;
    [g{s}] = rotate_IR_data(f{s},angle);
      
    % Convert intensity to temperature:
    temperature{s} = IntensityTempConv(thermalParam.emissivity,g{s}(:,:,rng{s}),seq{s});
...
switch plotType
    case 1
        options.frames = 95:2:size(temperature{s},3)-25;
        options.zlim = [0,150];
