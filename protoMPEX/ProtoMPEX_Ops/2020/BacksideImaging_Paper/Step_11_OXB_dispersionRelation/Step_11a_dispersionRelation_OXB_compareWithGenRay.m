% Comparing analytical dispersion with GENRAY produced one:
...
% FLR term:
L = 0.5*(linspace(1E-4,250,500).^2)*(b/Y)^2;
...
X_Bmode = 1./f;
n_perp_Bmode = sqrt(2*L*(Y/b)^2);

figure; 
plot(X_Bmode,n_perp_Bmode)
ylim([0,max(n_perp_Bmode)])
xlim([0,max(X)])
...
if 0
    ...
    for ss = 1:numel(X)
        ...
        n_perp_Bmode_u(ss) = sqrt(2*L_Bmode(ss)*(Y/b)^2);
    end
end
...
for ii = 1:numel(N_par)
    ...
    % B-mode:
    rng_Bmode = find(n_perp_Bmode > 8);
    hN(7) = plot(X_Bmode(rng_Bmode),n_perp_Bmode(rng_Bmode),'bl','lineWidth',3);
%     plot(X,n_perp_Bmode_u,'bl','lineWidth',3);

    % O-mode cutoff:
    hN(3) = line([1,1],[1E-2,1e3]);
    set(hN(3),'color','r','lineStyle','--','lineWidth',2)

    % Right hand cutoff
    hN(4) = line([1,1]*X_R,[1E-2,1e3]);
    set(hN(4),'color','k','lineStyle','--','lineWidth',2)

    % Left hand cutoff:
    hN(5) = line([1,1]*X_L,[1E-2,1e3]);
    set(hN(5),'color','k','lineStyle','--','lineWidth',2)

    % UH resonance layer:
    hN(6) = line([1,1]*X_UHR,[1E-2,1e3]);
    set(hN(6),'color','g','lineStyle','--','lineWidth',2)

    set(gca,'YScale','log')
    set(gca,'fontName','times','fontSize',12)
    xlabel('$ \omega_{pe}/\omega$','interpreter','latex','fontSize',17)
    ylabel('$ n_{\perp} $','interpreter','latex','fontSize',17)
    grid on 
...
hT{3} = text(0.40,0.012,'FX-cutoff','fontSize',fontSize.label,'interpreter','latex','color','k','rotation',90,'FontWeight',fontWeight);
hT{4} = text(0.76,0.012,'UHR','fontSize',fontSize.label,'interpreter','latex','color','g','rotation',90,'FontWeight',fontWeight);
...
hT{8} = text(0.49,211,'B-mode','fontSize',fontSize.label,'interpreter','latex','color','bl','rotation',0,'FontWeight',fontWeight);
...
% Oblique angle of injection:
figure(3)
title(['Incidence angle = ',num2str( asind(N_par(2)),2),'$^{\circ}$, ','$ \omega_{ce}/\omega$ = ',num2str(Y,2)],'fontSize',fontSize.title,'interpreter','latex');
hT{1} = text(0.4,0.9,'O-mode','fontSize',fontSize.label,'interpreter','latex','color','r','FontWeight',fontWeight,'rotation',-8);
hT{2} = text(0.1,0.3,'FX','fontSize',fontSize.label,'interpreter','latex','color','k','FontWeight',fontWeight);
hT{3} = text(0.34,0.012,'FX-cutoff','fontSize',fontSize.label,'interpreter','latex','color','k','rotation',90,'FontWeight',fontWeight);
hT{4} = text(0.76,0.012,'UHR','fontSize',fontSize.label,'interpreter','latex','color','g','rotation',90,'FontWeight',fontWeight);
