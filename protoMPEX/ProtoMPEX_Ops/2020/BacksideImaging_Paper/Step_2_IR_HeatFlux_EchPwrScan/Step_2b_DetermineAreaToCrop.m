...
shot     = [29671,29669,29665,29668,29661,29663,29662,29673,29655,29657];
pwr28GHz   = [5.92 ,26.12,36.84,38.29,45.41,50.98,52.61,54.07,70.07,70.48];
...
switch shot(kk)
    case {29671,29669,29665,29668,29661,29663,29662,29673,29655,29657}
        % Apply rotation:
        rotate_flag = 1;
...
options.n_Before = 10;
options.n_After = 10;
[intf,~,t0] = GetDataDuringPlasma(f,t_f,options);
...
if plotFig_1
    ...
    for kk = shotlist
        ...
        legendText{kk} = ['kk = ',num2str(kk),', ',num2str(shot(kk)),', 28 GHz power: ',num2str(pwr28GHz(kk))];
    end
    ...
    % Save figure:
    if saveFig_1
        figureName = 'Step_2b_IntegratedIntensityVsTime';
...
% Save figure:
if saveFig_2
    figureName = 'Step_2b_TemperatureMaps';
    saveas(gcf,figureName,'tiffn')
end

%% Save data:
if saveData
    varList = {'shot','pwr28GHz','seq','dT','t_dT','intf_c'};
    fileName = 'step_2b_dT_Cropped.mat';
