% Step_16a: microwave heating from the top
...
for s = 1:length(shot)
    ...
    extractOptions.frames = 1:1:150;
    extractOptions.frameRate = 60;
...
%     rng{s} = 1:size(f{s},3);
    ii0 = find(movmean(diff(intf{s}),10)>1,1);
    rng{s} = 1:(ii0+80);
...
if 1
    ...
    switch plotType
        case 1
            ...
            PlayMovieFromArray(temp_c{s},options)
        case 2
            options.frames = 1:2:size(temp{s},3)-2;
...
%% Plot data: 2D heat flux distributions:
% =========================================================================
...
for r = fr; %[1:5:length(intf_c)-3,fr]

    if timeAverage
            zz = ( q_IJR(:,:,r) + q_IJR(:,:,r+1) + q_IJR(:,:,r+2) )/3 - substractFlag*q_IJR(:,:,fr_substract); 
    else
            zz = q_IJR(:,:,r) - substractFlag*q_IJR(:,:,fr_substract); 
    end

    if useSurf == 1
        surf(xx*1e3,yy*1e3,movmean(movmean(zz,3,1),3,2),'LineStyle','none')
    elseif useContour1 == 1
        zLevels = linspace(zMin,zMax,15);
        [cCon,hCon] = contourf(xx*1e3,yy*1e3,zz,zLevels);
        set(hCon,'LineStyle','none');
    end

    % Plot LUFS:
    hold on
    hCirc(2) = plot3(xCirc*10 + 1 ,yCirc*10 + 1,ones(size(xCirc))*zMax,'k','LineStyle',':','LineWidth',3);
    ...
    title(['shot: ',num2str(shot),' $q(x,y,t)$ [MWm$^{-2}$]'],'interpreter','latex','fontSize',fontSize.title)
    ylabel('$y$ [mm]','interpreter','latex','fontSize',fontSize.label)
    xlabel('$x$ [mm]','interpreter','latex','fontSize',fontSize.label)
...
% Smooth data:
frame_length = 10;
zz = movmean(zz,frame_length,1);
zz = movmean(zz,frame_length,2);
...
if 0
    ...
    for ii = 1:size(zz,1)
        for jj = 1:size(zz,2)
            if heatFlux_peaked(ii,jj) < qThreshold
                heatFlux_peaked(ii,jj) = 0;
            else
                heatFlux_halo(ii,jj) = 0;
            end
            
            if heatFlux_halo(ii,jj) < 0.0
                heatFlux_halo(ii,jj) = 0;
            end
        end
    end
    ...
    % Smooth data:
    frame_length = 10;
    heatFlux_peaked = movmean(heatFlux_peaked,frame_length,1);
    heatFlux_peaked = movmean(heatFlux_peaked,frame_length,2);
    
    % Contour line:
    C = contour(xx,yy,zz,[1,1]*qThreshold);
    xx_peaked = C(1,2:end);
    yy_peaked = C(2,2:end);
    
    % Remove large numners:
    rng_rmv = find(abs(xx_peaked) > 1);
    xx_peaked(rng_rmv) = [];
    yy_peaked(rng_rmv) = [];
    
    rng_rmv = find(abs(yy_peaked) > 1);
    xx_peaked(rng_rmv) = [];
    yy_peaked(rng_rmv) = [];
    
    % Intergrate power [W]:
    pwr_peaked = sum(sum(heatFlux_peaked*1e6))*dxx*dxx
    pwr_halo   = sum(sum(heatFlux_halo*1e6))*dxx*dxx
   
    pwr_peaked/(pwr_halo + pwr_peaked)
end

% Plot hot spot contour line:
hold on
plot(xx_peaked*1e3,yy_peaked*1e3,'g','LineWidth',3)
...
% =========================================================================
if saveFig
    figureName = ['Step_16a_heatFlux_InjectionFromTop'];
    % PDF figure:
...
%% Plot data: Temporal evolution of heating rate:
% =========================================================================
...
if 1
    ...
    % =========================================================================
    if saveFig
        figureName = ['Step_16a_integratedHeatFlux_InjectionFromTop'];
