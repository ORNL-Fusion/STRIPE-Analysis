...
shotlist = 28000 + [928,1051,1065];
% shotlist = 29000 + [54,55,56];
shotlist = 29000 + [96,99];
shotlist = 29000 + 107;
...
for s = 1:length(shotlist)
    subplot(3,1,s)
    hold on
    h(1) = plot(t_f1{s}(1:end-1),(f1{s}-b*min(f1{s}))*20,C1{s},'LineWidth',1)
    h(2) = plot(t_f2{s}(1:end-1),(f2{s}-b*min(f2{s}))*20,C2{s},'LineWidth',1) % Ground Side
    h(3) = plot(t_f3{s}(1:end-1),(f3{s}-b*min(f3{s}))*20,C3{s},'LineWidth',1) % High Voltage side
    h(4) = plot(t_f4{s}(1:end-1),(f4{s}-b*min(f4{s}))*20,C4{s},'LineWidth',1)
    legend(h,'1','2','3','4')
%     ylim([0,30])
