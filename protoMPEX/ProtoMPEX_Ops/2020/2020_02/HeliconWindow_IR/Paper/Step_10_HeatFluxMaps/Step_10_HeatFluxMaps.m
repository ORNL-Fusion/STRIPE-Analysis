...
%% Write ONLY final-frame heat-flux CSV for Shot Series 1 and 5
% =========================================================================

outputCsvName = fullfile(dataAddress,...
    'heatFlux_theta_z_FINALFRAME_series1_series5.csv');
...
for kk = shotSeriesForCSV
    ...
    % OPTION 1: Explicit frame 40
    fr = min(40,size(S{kk}.qnorm,3));

    % OPTION 2: Last frame
    % fr = size(S{kk}.qnorm,3);

    heatFlux_W_m2  = S{kk}.qnorm(:,:,fr)*q0;
    heatFlux_kW_m2 = heatFlux_W_m2*1e-3;
    ...
    Time_s         = [Time_s; ...
        S{kk}.t_qnorm(fr)*S{kk}.t_star*ones(nRows,1)];

    Theta_deg      = [Theta_deg; thetaDeg(validIdx)];
    Z_m            = [Z_m; z_m(validIdx)];

    HeatFlux_W_m2  = [HeatFlux_W_m2; ...
        heatFlux_W_m2(validIdx)];

    HeatFlux_kW_m2 = [HeatFlux_kW_m2; ...
        heatFlux_kW_m2(validIdx)];

end

Tout = table(CaseName,ShotSeries,Frame,Time_s,...
    Theta_deg,Z_m,...
    HeatFlux_W_m2,HeatFlux_kW_m2,...
    'VariableNames',...
    {'Case','ShotSeries','Frame','Time_s',...
    'Theta_deg','Z_m',...
    'HeatFlux_W_m2','HeatFlux_kW_m2'});

writetable(Tout,outputCsvName)

disp(['Final-frame CSV written: ',outputCsvName])

%% Plot explicit final-frame heat-flux maps for Shot Series 1 and 5
% =========================================================================

seriesToPlot = [1,5];

seriesLabel = cell(1,5);
seriesLabel{1} = 'Series 1 - MPEX limiter-limited';
seriesLabel{5} = 'Series 5 - Window-limited';

figure('color','w','Name','Final-frame heat-flux maps: Series 1 and 5')

for ii = 1:numel(seriesToPlot)

    kk = seriesToPlot(ii);

    % Use frame 40, or last available frame if fewer than 40 exist
    frPlot = min(40,size(S{kk}.qnorm,3));
...
saveas(gcf,'FinalFrame_HeatFlux_Series1_vs_Series5','tiffn')

%% Cross-check: Original vs CSV reconstructed final-frame heat-flux maps
% =========================================================================

csvFileName = outputCsvName;

Tcsv = readtable(csvFileName);
...
figure('color','w', ...
       'Name','Original vs CSV final-frame cross-check', ...
...
for ii = 1:numel(seriesToPlot)
    ...
    qOriginal = S{kk}.qnorm(:,:,frPlot)*S{kk}.q0*1e-3;
    ...
    title({[seriesLabel{kk},' ORIGINAL'], ...
        ['Frame ',num2str(frPlot),', t = ',num2str(time_s,'%.4e'),' s']}, ...
        'Interpreter','none')
    ...
    % =====================================================================
    % CSV RECONSTRUCTED DATA
    % =====================================================================

    idx = Tcsv.ShotSeries == kk & ...
          Tcsv.Frame == frPlot;

    thetaCSV = round(Tcsv.Theta_deg(idx),6);
    zCSV_cm  = round(Tcsv.Z_m(idx)*1e2,6);
    qCSV     = Tcsv.HeatFlux_kW_m2(idx);

    theta_1D = unique(thetaCSV);
    z_1D     = unique(zCSV_cm);

    [theta_2D,z_2D] = meshgrid(theta_1D,z_1D);

    [~,thetaIdx] = ismember(thetaCSV,theta_1D);
    [~,zIdx]     = ismember(zCSV_cm,z_1D);

    qCSV_2D = accumarray([zIdx,thetaIdx],qCSV, ...
        [numel(z_1D),numel(theta_1D)],@mean,NaN);

    subplot(2,2,2*(ii-1)+2)

    surf(theta_2D,z_2D,qCSV_2D,'LineStyle','none')
    ...
    title({[seriesLabel{kk},' CSV'], ...
        ['Frame ',num2str(frPlot)]}, ...
        'Interpreter','none')
...
saveas(gcf,'CrossCheck_FinalFrame_Original_vs_CSV','tiffn')

%% Write separate final-frame CSV files for limiter-limited and window-limited cases
% =========================================================================
% Output columns:
% Theta_deg, Z_m, HeatFlux_W_m2, HeatFlux_kW_m2
...
for kk = shotSeriesForCSV
    ...
    switch kk
        ...
        case 1
            ...
            outputCsvName = fullfile(dataAddress,...
                'heatFlux_limiterLimited_series1_frame40.csv');

        case 5

            outputCsvName = fullfile(dataAddress,...
                'heatFlux_windowLimited_series5_frame40.csv');
    ...
    % Use frame 40 or last available frame
    fr = min(40,size(S{kk}.qnorm,3));
    ...
    Theta_deg      = thetaDeg(validIdx);
    Z_m            = z_m(validIdx);

    HeatFlux_W_m2  = heatFlux_W_m2(validIdx);
    HeatFlux_kW_m2 = heatFlux_kW_m2(validIdx);
    ...
    disp(['CSV written: ',outputCsvName])

end

%% Cross-check: Original vs separate CSV reconstructed heat-flux maps
% =========================================================================

seriesToPlot = [1,5];

seriesLabel = cell(1,5);
seriesLabel{1} = 'Limiter-limited, Series 1';
seriesLabel{5} = 'Window-limited, Series 5';

csvFileName = cell(1,5);
csvFileName{1} = fullfile(dataAddress,...
    'heatFlux_limiterLimited_series1_frame40.csv');
csvFileName{5} = fullfile(dataAddress,...
    'heatFlux_windowLimited_series5_frame40.csv');

figure('color','w', ...
       'Name','Original vs separate CSV cross-check', ...
       'Position',[100 100 1400 900])
...
for ii = 1:numel(seriesToPlot)
    ...
    Tcsv = readtable(csvFileName{kk});

    thetaCSV = round(Tcsv.Theta_deg,6);
    zCSV_cm  = round(Tcsv.Z_m*1e2,6);
    qCSV     = Tcsv.HeatFlux_kW_m2;
...
saveas(gcf,'CrossCheck_SeparateCSV_Original_vs_CSV','tiffn')
