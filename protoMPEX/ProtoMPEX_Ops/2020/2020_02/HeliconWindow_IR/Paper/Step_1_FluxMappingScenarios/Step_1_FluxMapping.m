% Illustrates the two different magnetic field geometries that are the
% focus of the investigation during the IR measurements of the helicon
% window: MPEX-limiter and Window-limiter.
...
% Skimmer:
fields.String = 'Skimmer';
x = 2.26*[1  ,1   ];
y =      [0.075,0.065];
hta = myTextArrow(gca,x,y,fields);

% Vertical line showing where the DLP measurement was performed:
hLine = line(1.2*[1,1],0.074*[-1,+1]);
hLine.Color = 'k';
hLine.LineStyle = ':';
hLine.LineWidth = 2;
...
set(hLeg,'FontSize',legendFontSize,'Location','best')
hLeg.Position = [0.16 0.7581 0.4315 0.0933];
hLeg.Box = 'off';

% Magnetic field profiles:
% =========================================================================
figureName = 'Step_1_MagneticFieldProfile';
figure('color','w','Tag',figureName)
hold on
for ii = 1:numel(coilCurrents)
...
set(hT,'color','k','LineWidth',4)
text(z_Target-0.4,0.35,'Target','interpreter','Latex','fontSize',12);

% Dump:
hT = line(z_Dump*[1,1],[0,0.3]);
set(hT,'color','k','LineWidth',4)
text(z_Dump-0.3,0.35,'Dump','interpreter','Latex','fontSize',12)
