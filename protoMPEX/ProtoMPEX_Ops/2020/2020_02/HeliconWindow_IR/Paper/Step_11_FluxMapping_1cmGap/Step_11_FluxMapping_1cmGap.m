% Objective:
% Illustrate the 1 cm gap required to significantly reduce the surface heat
% fluxes on the dielectric window
...
% =========================================================================
figureName = 'Step_11_FluxMapping_1cmGap';
...
% 1 cm gap annotations:
hL(1) = line([1.25,heliconWindow.z(1)],heliconWindow.r(2)*[1,1]);
hL(2) = line([1.25,1.749             ],           0.05313*[1,1]);
set(hL,'color','k')
fields.String = '$>$ 1 cm';
fields.HeadStyle = 'vback2';
x = 1.27*[1,1];
y = [0.99*hL(1).YData(1),hL(2).YData(1)];
hta = myTextArrow(gca,x,y,fields);
fields.String = [];
hta = myTextArrow(gca,x,fliplr(y),fields);

% Bulk plasma:
text(1.5,0.02,'Bulk plasma','Interpreter','Latex','FontSize',textArrowFontSize,'EdgeColor','k',...
    'BackgroundColor','w')

% Legend:
hLeg = legend(hFlux,legendText);
set(hLeg,'FontSize',legendFontSize,'Location','NorthWest')
hLeg.Position = [0.14 0.82 0.4315 0.0933];
hLeg.Box = 'off';
