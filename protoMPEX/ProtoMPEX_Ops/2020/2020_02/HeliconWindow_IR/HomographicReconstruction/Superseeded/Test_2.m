...
for kk = seriesToAnalyze;
    for jj = 1:numel(shotSeries{kk})
        ...
        ar = col/row;
        f{kk}{jj}.x_im = linspace(-0.5,0.5,row);
        f{kk}{jj}.y_im = linspace(-0.5*ar,0.5*ar,col);
        
    end
end

%% 5- Stitch images together: Pit side
% =========================================================================
close all
for kk = seriesToAnalyze;
    for jj = 1:6;
...
switch kk
    case 1
       ...
       bot_rng   = [001:162];
       bot_shift = 5;
       Bp = [circshift(im{kk}{3}(bot_rng,:),bot_shift,2);...
       circshift(im{kk}{2}(mid_rng,:),mid_shift,2);...
       circshift(im{kk}{1}(top_rng,:),top_shift,2)];
    case 2
end
...
box on
view([0,90])
set(gca,'PlotBoxAspectRatio',[colB/rowB 1 1])
xlim([0,size(Bp,2)])
ylim([0,size(Bp,1)])
title('Pit side')
caxis([0,15])

%% 5- Stitch images together: non-Pit side
% =========================================================================
% close all
...
% Stitch raw data:
Anp = [circshift(im{6}(:,:),000,2);...
       circshift(im{5}(:,:),000,2);...
       circshift(im{4}(:,:),000,2)];

[rowAnp,colAnp] = size(Anp); 
 
figure
surf(Anp,'LineStyle','none')
box on
view([0,90])
set(gca,'PlotBoxAspectRatio',[colAnp/rowAnp 1 1])
xlim([0,size(Ap,2)])
ylim([0,size(Ap,1)])
title('non-Pit side')
...
switch cropType
    case 1
Bnp = [circshift(im{6}(001:163,:),000,2);...
      circshift(im{5}(065:168,:),000,2);...
      circshift(im{4}(063:end,:),-08,2)];
    case 2
end

[rowBnp,colBnp] = size(Bnp); 
 
figure
surf(Bnp,'LineStyle','none')
box on
view([0,90])
set(gca,'PlotBoxAspectRatio',[colBnp/rowBnp 1 1])
xlim([0,size(Bnp,2)])
ylim([0,size(Bnp,1)])
title('non-Pit side')
caxis([0,15])
...
xlabel('y')
ylabel('x')
set(gca,'PlotBoxAspectRatio',[ar 2 1])
xlim([-1,1]*max(y_im))
ylim([-2,2]*max(x_im))
