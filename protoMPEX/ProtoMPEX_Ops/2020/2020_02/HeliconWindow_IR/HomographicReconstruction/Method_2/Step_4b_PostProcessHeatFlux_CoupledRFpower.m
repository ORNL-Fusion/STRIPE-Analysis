...
if 1
    figureName = ['Heatflux_Partial2D_Extrapolated_175kW_Coupled'];
    saveas(gcf,figureName,'tiffn')
end
...
if 1
    figureName = ['Heatflux_complete2D_Extrapolated_175kW_Coupled'];
    saveas(gcf,figureName,'tiffn')
end
...
d.R_vacuum = Rv;
d.R_plasma = Rp;
d.RFpower_NET = mean_RF_NET
d.RFpower_Coupled = mean_RF_Coupled

% Geometry of window:
d.RadiusWindow = 0.0620; 
d.LengthWindow = 0.3;
d.MagneticFluxMapping = 'MPEX-limiter'
...
d.comment{2} = 'Heat flux has been linearly extrapolated from experiments performed at 80, 120 and 140 kW NET power in Proto-MPEX';
...
fileName = ['HeatFlux2D_ExtrapolatedTo_175kW_CoupledPwr_created_',date];
