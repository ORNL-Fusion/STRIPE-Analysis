...
% =========================================================================
load Step_4c_MPEX_ICH_HeatFlux2D_155kW_CoupledPwr_created_26-Feb-2021.mat
...
for ii = 1:numel(dataSet_ICH)
    dz = mean(diff(z(:,1)));
    ds = dataSet_ICH{ii}.RadiusWindow*mean(diff(phi(1,:)));
    TotalPower = sum(sum(heatflux{ii}))*dz*ds;
    disp(['total power of ',dataSet_ICH{ii}.MPEX_sealType,' is ',num2str(TotalPower*1e-3),' [kW]'])
end
