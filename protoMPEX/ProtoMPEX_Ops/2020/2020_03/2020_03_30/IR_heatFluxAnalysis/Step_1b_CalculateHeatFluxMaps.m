...
for kk = shotsToAnalyze
    ...
    % Save figure:
    figureName = ['Step_1b_FourierRepresentation_',num2str(round(pwrECH(kk))),'_kW_',num2str(shot(kk))];
    ...
    if kk == 10
        % Save figure:
        figureName = ['Step_1b_FourierContent_',num2str(shot(kk))];
        saveas(gcf,figureName,'tiffn')
    end
...
for kk = shotsToAnalyze
    ...
    for r = [1:5:length(intf_c{kk})-3,fr]

        if timeAverage
                zz = ( q_IJR{kk}(:,:,r) + q_IJR{kk}(:,:,r+1) + q_IJR{kk}(:,:,r+2) )/3 - substractFlag*q_IJR{kk}(:,:,fr_substract); 
        else
                zz = q_IJR{kk}(:,:,r) - substractFlag*q_IJR{kk}(:,:,fr_substract); 
        end
        ...
        % Plot LUFS:
        hold on
        hCirc(2) = plot3(xCirc*10 - 3 ,yCirc*10,ones(size(xCirc))*zMax,'k','LineStyle',':','LineWidth',2);
        ...
        % title(['shot: ',num2str(shot(kk)),' , frame: ',num2str(r),', [MWm$^{-2}$]'],'interpreter','latex','fontSize',fontSize.title)
        title(['shot: ',num2str(shot(kk)),', Target heat flux [MWm$^{-2}$]'],'interpreter','latex','fontSize',fontSize.title)
    ...
    % Save figure:
    figureName = ['step_1b_HeatFluxMap_',num2str(round(pwrECH(kk))),'_kW_shot_',num2str(shot(kk))];
    ...
    % Save figure:
    figureName = ['step_1b_IntegratedPower_',num2str(round(pwrECH(kk))),'_kW_shot_',num2str(shot(kk))];
    saveas(gcf, figureName,'tiff')
end
...
data.pwrECH = pwrECH;
% ###############################################
% The following data is not available yet:
...
% -------------------------------------------------------------------------
data.integratedEmission = intf_c;

% Total power coupled to the Target:
% -------------------------------------------------------------------------
data.pwr = pwr;

% Save data:
% -------------------------------------------------------------------------
fileName = ['Step_1b_heatFlux2D.mat'];
save(fileName,'data');
