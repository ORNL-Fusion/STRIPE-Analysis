...
% load data:
% =========================================================================
load('Step_1_RawData_2020_03_31.mat')
...
% Plasma radius:
rprobe = RawData.MetaData.rgauge - 11.25;
...
% -------------------------------------------------------------------------
r_ne_ss = r_ne_m;
rng = find(t_ne_m > 4.58 & t_ne_m < 4.64);
ne_ss = mean(ne_m(rng,:));
...
line(+[1,1]*rLUFS,[0,6],'color','g','LineStyle','--','LineWidth',2)
hLine = line(-[1,1]*rLUFS,[0,6],'color','g','LineStyle','--','LineWidth',2)
xlim([-4,4])
ylim([0,6])
box on
