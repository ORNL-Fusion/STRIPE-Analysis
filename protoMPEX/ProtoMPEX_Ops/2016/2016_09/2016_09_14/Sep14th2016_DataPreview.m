...
[ICH,t_ICH] = my_mdsvalue_v3(shotlist,Data);
...
for s = 1:length(shotlist)
    ...
    L{s} = [num2str(shotlist(s))];
    plot(t_G{s},G{s}*5e18,'g','lineWidth',1)
    plot(t_ICH{s},ICH{s}*-40*5e18,C{s},'lineWidth',0.3,'LineStyle',':')
end
...
for s = 1:length(shotlist)
    ...
    plot(t_G{s},G{s},'g','lineWidth',1)
    plot(t_ICH{s},ICH{s}*-40,C{s},'lineWidth',0.3,'LineStyle',':')
...
for s = 1:length(shotlist)
    ...
    plot(t_G{s},G{s}*3,'g','lineWidth',1)
    plot(t_ICH{s},ICH{s}*-40*3,C{s},'lineWidth',0.3,'LineStyle',':')
