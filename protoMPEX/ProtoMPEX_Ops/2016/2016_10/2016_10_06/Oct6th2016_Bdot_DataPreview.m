...
shotlist = 1e4 + 700 + [60,61,62,63];


shotlist = 1e4 + 700 + [58:60,61,62,63,64];

% Sequence for Btheta scan:
shotlist = 1e4 + 700 + [69:97];
R = [3.5, 4, 4.5, 5, 5.5, 6*ones(1,5), 6.5, 7, 7.5, 7.5, 8, 8.5,...
    9, 9.5, 10, 10, 10.5, 10.5, 11, 11.5, 12, 12.5, 13, 13.5, 14];
...
for s = 1:length(shotlist)
    hP(s) = plot3(tBdot{s},R(s)*ones(size(P{s})),P{s});
end
title('Phase')
...
for s = 1:length(shotlist)
    ...
    hM(s) = plot3(tBdot{s},R(s)*ones(size(VmagRatio{s})),VmagRatio{s});
end
title('Magnitude')
