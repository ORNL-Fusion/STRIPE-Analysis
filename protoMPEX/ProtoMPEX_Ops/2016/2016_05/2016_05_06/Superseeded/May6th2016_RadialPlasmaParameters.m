...
figure;
hold on
y = [0,0.5,1,1.5,2,2.5,3,3.5,-0.5,-1,-1.5,-2];
for s = 1:length(Shots)
    plot3(time{s},y(s)*ones(size(Ni{s})),Ni{s})
end
view([30,30])

figure; hold on
for s = 1:length(Shots)
    plot3(time{s},y(s)*ones(size(Te{s})),sgolay_t(Te{s},3,7))
end
zlim([0,10])
view([30,30])
...
figure; hold on
for s = 1:length(Shots)
    plot3(time{s},y(s)*ones(size(Te{s})),e_c*Ni{s}.*sgolay_t(Te{s},3,7))
end
zlim([0,70])
view([30,30])
%%
