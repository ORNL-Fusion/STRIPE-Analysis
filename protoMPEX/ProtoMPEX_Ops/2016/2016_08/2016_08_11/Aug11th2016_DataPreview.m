...
for s = 1:length(Shots)
    ...
    if 1
        Im{s} = sgolay_t(Im{s},3,21);
        Vm{s} = sgolay_t(Vm{s},3,21);

    end
    Im{s} = -2*Im{s}(rng)/150; % V = IR, I = V/R 150 Ohms resistor and a 2:1 voltage divider
    tm = 2;
...
ylim([0,10e19])
ax(1) = gca;
legend([h],'6300 A','6400 A','6500 A','No ECH','location','SouthEast')
...
for s = 1:length(Shots)
    ...
    %plot(t_Gyrotron{s},0.6*Gyrotron{s},C(s))
end
    case 2
end
...
ylim([0,10e19])
ax(1) = gca;
legend([h],'6300 A','6400 A','6500 A','location','SouthEast')
