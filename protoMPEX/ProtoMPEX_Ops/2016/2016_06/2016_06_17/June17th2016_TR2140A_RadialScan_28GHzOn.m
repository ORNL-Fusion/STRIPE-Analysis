...
%Extract data:
Spool = 9.5;
Shots = 9000 + [67:70,73,74,83,84,86,88,90,91,92,93];
Radius = [0:0.5:4.0,-0.5:-0.5:-2.5];
Stem = '\MPEX::TOP.';
Branch = 'MACHOPS1:';
RA = [Stem,Branch];
...
figure; hold on
for s = 1:length(Shots)
    rng = find(time{s}>=4.27 & time{s}<=4.31);
...
set(gca,'Box','on')
legend(h,'Spool 6.5, TR2 = 140 A, PS1 = 5700 A','Location','NorthWest')
set(gca,'XTick',[-5:1:5])
...
figure; hold on
for s = 1:length(Shots)
    rng = find(time{s}>=4.24 & time{s}<=4.26);
    plot3(time{s}(rng),Radius(s)*ones(size(Ni{s}(rng))),Ni{s}(rng),C(s))
    Ni_mean(s) = mean(Ni{s}(rng));
    dNi_mean(s) = std(Ni{s}(rng));
end
