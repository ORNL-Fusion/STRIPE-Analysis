...
switch ShotType
    case 1 
        shotlist = [31000 + [353,358]]; 
        shotlist = [31000 + [353,369,372,374]]; 
        shotlist = [31000 + [761,768]]; 
 
        DLPType = '6MP';
    case 2
        shotlist = [31000 + [502,503,504,505,506]]; 
        shotlist = [31000 + [497,506,507,508,510,511,513]]; 
        shotlist = [31000 + [181,497,514,515,516,517]]; 

       shotlist = [31000 + [181,517,518,519]]; 
       shotlist = [31000 + [532,533,534]]; 
       shotlist = [31000 + [537,538,542,543]]; 
       shotlist = [31000 + [543,544,545,546,547,548]]; 
       shotlist = [31000 + [546,548,550,551]]; 
       shotlist = [31000 + [552]]; 
       shotlist = [31000 + [553]]; 
    ...
    case 3
       shotlist = [31000 + [772,773,774,777,778]]; 
       shotlist = [31000 + [783,784,785,786,787,788 ]]; 
       R        =          [12 ,10 ,8  ,6  ,5  ,4.25] ;
...
Config.tEnd = Config.tStart + 1.75;
...
switch DLPType 
    ...
    case '6MP'
        ...
        Config.L_tip = 2.0/1000; % 2.0 as of 2020_03_09
        Config.D_tip = 0.254/1000; % [m]
    case '10'
        DLP = 10.5;
...
%         Config.L_tip = 1.6/1000;% according to Nischal 2017_11_21
    ...
    case '1MP'
        ...
        Config.L_tip = 2.0/1000; % As of 2020_11_16
        Config.D_tip = 0.254/1000; % [m]
    case 'TDLP'
        DLP = 11.5;
        Config.L_tip = 0/1000;
...
for s = 1:length(shotlist)
    GoodFits{s} = GlitchFlag{s} == 0 & StdResNorm{s}<=0.6 & Ni{s}>0 & Ni{s}<50e19 & Te{s}<=30 & Is{s}>0.01e-3;
...
%     y = y - mean(y(1:100));
%     y = (y./max(y))*yMax*0.75;
%     plot(t_ech{s}(1:length(ECH{s})),y,C{s},'lineWidth',0.5)
%     
%     y = abs(RF{s});
%     y = y - mean(y(1:100));
%     y = (y./max(y))*yMax*0.75;
%     plot(t_rf{s}(1:length(RF{s})),y,C{s},'lineWidth',1.5)
    
%     y = abs(ICH_F{s});
%     y = (y./max(y))*yMax*0.75;   
%     plot(t_ICH_F{s}(1:length(ICH_F{s})),y,C{s},'lineWidth',1.5)
end
...
save('highPriorityShots_2020_02_19','shotlist','t_Te_gf','Te_gf','t_Ni_gf','Ni_gf')
end
...
if 1
    ...
    errorbar(R-0*12,nes{3},dnes{3},'ko')
    errorbar(R-0*12,nes{2},dnes{2},'ro')
    ylim([0,9e19])
    xlim([0,15]-0*7.5)
    
    subplot(2,1,2); hold on
    errorbar(R-0*12,Tes{3},dTes{3},'ko')
    errorbar(R-0*12,Tes{2},dTes{2},'ro')
    ylim([0,10])
    xlim([0,15]-0*7.7)
end
