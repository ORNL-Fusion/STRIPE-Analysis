...
switch limitType
    ...
    case 4 % Calculation for tommy S.
        coilCurrents.TR1 = 0;
        coilCurrents.TR2 = 520;
        coilCurrents.PS1 = 3770;
...
z_Dump = 0.5;
z_Target = 4.2;
r1D = linspace(1e-3,0.15  ,100 );
z1D = linspace(z_Dump,z_Target,1000);
...
xi_lines = linspace(0.1e-2,1,10).^2;
...
for ii = 1:1:numel(xi_lines)
    dum1 = plot((z_fluxline{ii}),+r_fluxline{ii},'k-','LineWidth',0.5);
    dum2 = plot((z_fluxline{ii}),-r_fluxline{ii},'k-','LineWidth',0.5);
...
% Spool 1.5:
line([1,1]*1.2,[-1,1]*0.2,'color','g','LineWidth',2)
...
for ii = 1:numel(coilCurrents)
    hBz(ii) = plot(z1D,B2D(:,1),lineColor{ii},'LineWidth',2);
end
box on 
grid on
...
Rp = 2/100;
Bz = B2D(:,1);
Br = -0.5*Rp*diff(Bz)/dz;

figure
hold on
plot(z1D(1:end-1),Br./Bz(1:end-1),'k')
line([1,1]*1.2,[-1,1]*0.1,'color','g')
