% June 16th 2017, Bdot probe scan at 4kA
...
shot = 15100 + [59,60,61,62,63,64,66,67,68,70 , 71];
R    =         [0 , 4, 5 ,6 , 7,8 ,9 ,10,11,7.5,6.5];

Data{1} = ['\MPEX::TOP.MACHOPS1:MN_CURRENT']; % Vp0
Data{2} = ['\MPEX::TOP.MACHOPS1:GEN_RF_PWR']; % Vp90
Data{3} = ['\MPEX::TOP.MACHOPS1:EA_CURRENT']; % Vmag 
...
for ch = 1:4 % For all channels on digitizer
    ...
    hold on
    for s = 1:length(shot)
            rng = find(t{ch}{s}>= 4.12 & t{ch}{s}<= 4.45);
            t{ch}{s} = t{ch}{s}(rng);
            if isempty(rng)
                rng = find(t{ch}{s+1}>= 4.12 & t{ch}{s+1}<= 4.45);
                t{ch}{s} = t{ch}{s+1}(rng);
            end
...
[VmagRatio,P,TBdot] = BdotPhaseDetect_v1(shot,Data);
    ...
    save('Bdot_2017_06_16')
else
    load('Bdot_2017_06_16')
end
