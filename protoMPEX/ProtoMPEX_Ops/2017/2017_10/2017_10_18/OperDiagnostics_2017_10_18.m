...
RemoveOffset = 0;
shotlist = 16430;
shotlist = 16000 + [485:489];
shotlist = 16000 + [500,505,506];
shotlist = 16000 + [714,958,974,991];
shotlist = [12619,16714];

close all
TR2_filt =         ones(size(shotlist));
TR2_ffrm =         ones(size(shotlist))*15;
Calshot  =         ones(size(shotlist))*15405;
PGcenter =         [];
...
for s = 1:length(shotlist)
    for p = 1:4
        ...
        PGc{1}{s} = (cell2mat(PG{1}{s})-0*cell2mat(PGcal{1}{s}))*2;
        t_pgc{1}{s} = cell2mat(t_pg{1}{s});
        PGc{2}{s} = (cell2mat(PG{2}{s})-0*cell2mat(PGcal{2}{s}))*2;
        t_pgc{2}{s} = cell2mat(t_pg{2}{s});
        PGc{3}{s} = (cell2mat(PG{3}{s})-0*cell2mat(PGcal{3}{s}))*2;
        t_pgc{3}{s} = cell2mat(t_pg{3}{s});        
        PGc{4}{s} = (cell2mat(PG{4}{s})-0*cell2mat(PGcal{4}{s}))*10;
