...
[ni,Te,time,Ifit,Ip,Vp,tm,Vsweep,Isweep] = DLP_fit_V5_2_2017_02_14(Config,shotlist,DataAddress);
...
for s = 1:length(shotlist)
%     PlotRng{s} = find(abs(StdResNorm{s})<10/100 & StdRes{s} == real(StdRes{s}));
%     if ~isempty(PlotRng{s})
%             h(s) = plot(time{s}(PlotRng{s}),Ni{s}(PlotRng{s}),C{s},'lineWidth',2);
