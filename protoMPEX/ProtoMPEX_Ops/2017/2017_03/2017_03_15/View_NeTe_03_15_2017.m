...
shotlist = 13200 + [75,76,77,78,79,80,81];
shotlist = 13200 + [80,81,84,85,86,87,88,89,90,92];
shotlist = 13200 + [92];
shotlist = 13200 + [100,101,104,105];
shotlist = 13200 + [98,100,101,105];
shotlist = 13200 + [102,104,105];
shotlist = 13200 + [71:74];

% all the shots on March 15th had a time trace with a -4 sec offset

address{1} = '\MPEX::TOP.MACHOPS1:PG4'; 
[PG4,t_pg4] = my_mdsvalue_v2(shotlist,address(1));
[PG4_0,t_pg4_0] = my_mdsvalue_v2(13303,address(1));

address{1} = '\MPEX::TOP.MACHOPS1:PG3'; 
[PG6,t_pg6] = my_mdsvalue_v2(shotlist,address(1));
[PG6_0,t_pg6_0] = my_mdsvalue_v2(13303,address(1));

address{1} = '\MPEX::TOP.MACHOPS1:RF_FWD_PWR'; 
[Prf,t_prf] = my_mdsvalue_v2(shotlist,address(1));

address{1} = '\MPEX::TOP.MACHOPS1:PS1_I'; 
[Ps1,t_ps1] = my_mdsvalue_v2(shotlist,address(1));

AddressType='s'; % s for standard
CalType='niso'; % niso for not isolated, iso for isolated
DLPType='9';

Config.tStart = 4.150 - 4; % [s]
Config.tEnd = 4.32 - 4; 
...
for s = 1:length(shotlist)
    ...
    hold on
    %plot(t_ech{s},ECH{s}*1e19,C{s},'lineWidth',0.5)
end
...
for s = 1:length(shotlist)
    plot(t_pg4{s},(PG4{s}-mean(PG4_0{1}(1:800)))*10,C{s},'lineWidth',2)
    plot(t_pg6{s},(PG6{s}-mean(PG6_0{1}(1:800)))*2,C{s},'lineWidth',1,'LineStyle',':')

end
ylim([0,20])
xlim([4.1,4.6])
title('$ PG4 $ $ [mTorr] $','interpreter','Latex','FontSize',13,'Rotation',0)
...
if 1
    ...
    hp(s) = plot(t_pg4{s}-4,PG4{s}*40,'r');
