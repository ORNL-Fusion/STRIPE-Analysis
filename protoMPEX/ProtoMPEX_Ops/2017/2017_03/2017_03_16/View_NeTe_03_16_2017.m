...
for s = 1:length(shotlist)
    plot(t_pg4{s}(1:length(PG4{s})),(PG4{s}-PG4_0{1})*10,C{s},'lineWidth',2)
    plot(t_pg6{s}(1:length(PG4{s})),(PG6{s}-PG6_0{1})*2,C{s},'lineWidth',1,'LineStyle',':')
...
if 1
    ...
    hp(s) = plot(t_pg4{s}(1:length(PG4{s})),PG4{s}*40,'r');
    hrf(s) = plot(t_prf{s}(1:length(Prf{s})),Prf{s}*100,'g');
    ylim([-100,100])
    xlim([4.15,4.32])
