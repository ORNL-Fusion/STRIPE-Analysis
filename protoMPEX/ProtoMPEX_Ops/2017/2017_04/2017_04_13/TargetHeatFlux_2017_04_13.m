...
for r = [1:5:70,fr]
    ...
    hT = title(['shot: ',num2str(shot(s)),' , frame: ',num2str(r), ', Heat flux [MWm$^{-2}$]']);
...
s = 1;
figure('color','w')
plot(movmean(pwr,3)*1e-3,'k','LineWidth',2)
box on
ylabel('[kW]','interpreter','Latex','Fontsize',12)
xlabel('frame $\propto$ time','interpreter','Latex','Fontsize',12)
title(['shot: ',num2str(shot(s))],'interpreter','Latex','Fontsize',12)
ylim([0,2])
xlim([1,45])

% Save figure:
% saveFig = 1;
% figureName = ['shot_',num2str(shot),'_PowerToTarget'];
% if saveFig
%     saveas(gcf,figureName,'tiffn')
% end
