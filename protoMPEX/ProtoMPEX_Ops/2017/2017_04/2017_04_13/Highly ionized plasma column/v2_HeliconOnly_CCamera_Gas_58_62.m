...
% RE1 = mean(RadialEmission(:,400:440),2);
dn = 10;
RE1 = mean(RadialEmission(:,rng(1:dn)),2);
plot(RE1(end:-1:1),1:length(RE1),'Parent',axvid(3),'color','k');
...
plot(RE2(end:-1:1),1:length(RE2),'Parent',axvid(3),'color','k');
text(axvid(3),0.38,0.5,'b)','Units','normalized','color','k','FontSize',10,'FontName','Times')

RE2_ECH = mean(RadialEmissionECH{2}(:,rng(n1:n2)),2);
plot(RE2_ECH(end:-1:1),1:length(RE2_ECH),'Parent',axvid(3),'color','r');
...
plot(i-340,RE2)
rng1 = i(340:end);
plot(i(rng1)-340,RE2(rng1),'r')
rng2 = i(340:-1:1)
plot(i(rng2)-340,RE2(rng2),'g')
...
[ f_rec , X ] = abel_inversion(y1(150:end),4,24,1,0);
%[ f_rec , X ] = abel_inversion(RE1(rng1),4,24,1,0);
