...
% DO NOT USE THIS CODE, IT HAS BEEN MODIFIED TO REMOVE THE EFFECTS OF
% GLITHCES BUT IT DID NOT WORK - 2018_03_28
...
% #########################################################################
[y1,y2,~,y3,y4,y5,y6,y7,y8,y9,y10,y11,y12,y13] = DLP_fit_V5_6(Config,shotlist(s),DataAddress);
...
%% Load Color camera and Baratron data for each location
...
RF_kW = RFnormalized*110; % kW based on the FRT-86 logbook shot = 13962
trfnorm = trf{1}(1:end-1);

h(1) = plot(trfnorm,RF_kW,'Parent',ax(n),'color','k');
...
% Labelling
ax(n).FontName = 'Times';
t1 = text(4.46,90,'RF power');
t1.FontName = 'Times';
ylabel(ax(n),'[kW]','FontSize',11);
...
% hne(2) = plot(time{s}(rng(nskip:end)),Te{s}(rng(nskip:end)),'Parent',ax(n),'color','k','LineStyle',':');
hne(2) = plot(time{s}(rng(nskip:end)),Te{s}(rng(nskip:end)),'Parent',ax(n),'color','k');
hne(1).LineWidth =  1.5;
ylim(ax(n),[0,8])
...
rngData{s} = find(StdResNorm{s}<=0.05);
hne(1) = plot(tne4(rngData{s}),ne4(rngData{s})*1e-19,'Parent',ax(n),'color','k')
hne(2) = plot(tTe4(rngData{s}(nskip:end)),Te4(rngData{s}(nskip:end)),'Parent',ax(n),'color','k')
hne(1).LineWidth =  1.5;
ylim(ax(n),[0,8])
...
hne(1) = plot(time{s}(rng),Ni{s}(rng)*1e-19,'Parent',ax(n),'color','k')
hne(2) = plot(time{s}(rng(nskip:end-1)),Te{s}(rng(nskip:end-1)),'Parent',ax(n),'color','k')
hne(1).LineWidth =  1.5;
ylim(ax(n),[0,8])
% Lne = legend(hne,'$n_e$','$T_e$');
% Lne = legend(hne,'$n_e{\times}10^{-19}$','$T_e$');
...
hne(2) = plot(tTe10(nskip:end-2),Te10(nskip:end-2),'Parent',ax(n),'color','k');
hne(1).LineWidth =  1.5;
ylim(ax(n),[0,8])
% Lne = legend(hne,'$n_e{\times}10^{-19}$','$T_e$');
% Lne.Interpreter = 'latex';
...
if 0
save('DLP_4_10_2017_04_13','RFnormalized','trfnorm','ne4','tne4','Te4','tTe4','ne10','tne10','Te10','tTe10')
end

% If we resave, we need to move the legend to the correct position
if 1 
RootAddress = cd;
print(figure(1),[RootAddress,'\Figure2'],'-dpng')
end
...
% Labelling
ax(n).FontName = 'Times';
% t1 = text(4.3,90,'Helicon');
t1a = text(4.46,90,'RF power');
t1a.FontName = 'Times';
ylabel(ax(n),'[kW]','FontSize',11);
ax(n).YLabel.Interpreter = 'latex';
ax(n).YLabel.Rotation = 90; 
...
yyaxis right
h(2) = plot(t_MFC{1}(1:end-1),MFC{1}-MFC_offset{1},'Parent',ax(n),'color','r','linewidth',2);
set(gca,'YColor','r')
ylim([0,2])
t1b = text(4.66,0.5,'$D_2$ flow rate','Interpreter','latex');
t1b.Color = 'r';
t1b.FontName = 'Times';
ylabel(ax(n),'[SLM]','FontSize',11);
ax(n).YLabel.Interpreter = 'latex';
ax(n).YLabel.Rotation = 90; 
ax(n).YTick = [1,2];
...
hpgA(2) = plot(tpg2{1}(1:end-1),PG2_GasOnly{1}/7.5,'Parent',ax(n),'color','k')
hpgA(1).LineWidth =  1.5;
ylim(ax(n),[-0.05,4])
ax(n).YGrid = 'on';
...
hpgA(2) = plot(tpg2{1}(1:end-1),PG4_GasOnly{1}/7.5,'Parent',ax(n),'color','k')
hpgA(1).LineWidth =  1.5;
ylim(ax(n),[-0.05,3])
ax(n).YGrid = 'on';
% LpgA = legend(hpgA,'w/ Plasma','no Plasma');
...
hpgB(2) = plot(tpg2{1}(1:end-1),PG3_GasOnly{1}/7.5,'Parent',ax(n),'color','k')
...
hpgB(2) = plot(tpg1{1}(1:end-1),PG1_GasOnly{1}/7.5,'Parent',ax(n),'color','k')
hpgB(1).LineWidth =  1.5;
ylim(ax(n),[0,0.4])
ax(n).YGrid = 'on';
...
for s = 1:length(ax)
    ax(s).YLabel.FontSize = 11;
    if s == 5
        text(ax(s),0.92,0.85,abc{s},'Units','normalized','Interpreter','latex','FontSize',11)
    else
        text(ax(s),1.01,0.85,abc{s},'Units','normalized','Interpreter','latex','FontSize',11)
    end
end
...
RootAddress = cd;
print(gcf,[RootAddress,'\Figure4'],'-dpng')
