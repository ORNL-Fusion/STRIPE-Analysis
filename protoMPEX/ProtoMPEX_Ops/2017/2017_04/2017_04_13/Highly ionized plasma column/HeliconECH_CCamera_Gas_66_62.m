...
% ECH. It shows Te increase while there is a drop in ne.
...
% Shots to use:
% 13966, 300 ms RF, 90 ms ECH at 4.2 sec, DLP 10.5
% 13962, 300 ms RF, no ECH, Probes out (DLP 6.5 at y = 4 cm)
...
if 1
    ...
    save('HeliconECH_CCamera_Gas_66_62.mat')
else
    load('HeliconECH_CCamera_Gas_66_62.mat')
end
%%
%##########################################################################
...
I_start = (S{2}(rng(1)).cdata); %rgb2gray
imh(1) = image(axvid(1),I_start);   %axvid(1) = gca;
...
I_end = (S{2}(rng(end)).cdata);
imh(2) = image(axvid(2),I_end); hold on %axvid(2) = gca;
...
plot(RadialEmission{2}(end:-1:1,rng(1)),1:length(RadialEmission{2}(:,rng(end))),'Parent',axvid(3),'color','k');
text(axvid(3),0.63,0.5,'a)','Units','normalized','color','k','FontSize',12,'FontName','Times')

plot(RadialEmission{2}(end:-1:1,rng(end)),1:length(RadialEmission{2}(:,rng(end))),'Parent',axvid(3),'color','k');
...
if 1
save('HeliconECH_66_62_Dataset_1','RF','trf','PG1','PG2','PG3','PG4','tpg1','tpg2','tpg3','tpg4'...
    ,'PG1_GasOnly','PG2_GasOnly','PG3_GasOnly','PG4_GasOnly','RadialEmission','OnAxisEmission','t','ECH','t_ech'...
    ,'I_start','I_end')
end
...
fvid.Color = 'w'
subplot(1,2,1)
surf(S{1}(rng(1)).cdata(:,:,1),'LineStyle','none')
zlim([0,200])
caxis([0,200])
axis('square')
view([0,90])
...
subplot(1,2,2)
surf(S{1}(rng(end)).cdata(:,:,1),'LineStyle','none')
zlim([0,200])
caxis([0,200])
axis('square')
view([0,90])
...
figure
surf((S{1}(rng(1)).cdata(:,:,1)-S{1}(rng(end)).cdata(:,:,1)),'LineStyle','none')
% zlim([-100,200])
% caxis([-100,200])
axis('square')
view([0,90])
