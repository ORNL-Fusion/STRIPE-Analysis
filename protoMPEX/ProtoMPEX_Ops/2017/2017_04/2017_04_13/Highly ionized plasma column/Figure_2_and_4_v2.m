...
ne4 = Ni{s}(rng);
tne4 = time{s}(rng);
rngData = [11:53,56:62,65:73,76:103]; % include data with acceptable fits
Te4 = Te{s}(rngData);
tTe4 = time{s}(rngData);

hne(1) = plot(tne4,ne4*1e-19,'Parent',ax(n),'color','k')
hne(2) = plot(tTe4,Te4,'Parent',ax(n),'color','k')
hne(1).LineWidth =  1.5;
ylim(ax(n),[0,8])
...
ne10 = Ni{s}(rng);
tne10 = time{s}(rng);
Te10 = Te{s}(rng(nskip:end-2));
tTe10 = time{s}(rng(nskip:end-2));
hne(1) = plot(tne10,ne10*1e-19,'Parent',ax(n),'color','k');
hne(2) = plot(tTe10,Te10,'Parent',ax(n),'color','k');
hne(1).LineWidth =  1.5;
ylim(ax(n),[0,8])
% Lne = legend(hne,'$n_e{\times}10^{-19}$','$T_e$');
% Lne.Interpreter = 'latex';
