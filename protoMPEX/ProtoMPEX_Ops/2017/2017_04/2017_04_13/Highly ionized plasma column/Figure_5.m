...
set(f,'Position',[0.6 0.1 0.3 0.4]); % [Left Bottom Width height]
...
ECH_kW = ECHnormalized*14; % kW based on 11.1 kW per volt
hRF(1) = plot(trf{1}(1:end-1),RF_kW,'Parent',ax(n),'color','k');
hRF(2) = plot(t_ech{1}(1:end-1),sgolay_t(ECH_kW,3,11)-1,'Parent',ax(n),'color','k');
hRF(2).LineWidth = 1.5;
ylim(ax(n),[0,120])
ax(n).YGrid = 'on';
% L5 = legend(hRF,'$P_{RF}$','28 GHz'); L5.Interpreter = 'latex'; L5.Box = 'off'; L5.Location = 'northwest';
t1 = text(4.3,95,'Helicon'); t1.FontName = 'Times';
t2 = text(4.3,25,'28 GHz'); t2.FontName = 'Times';
...
Lne.Interpreter = 'latex'; Lne.FontSize = 7;
Lne.Box = 'off'; Lne.Location = 'West';
ax(n).YGrid = 'on';
...
hpgB(2) = plot(tpg2{1}(1:end-1),PG3{2}/7.5,'Parent',ax(n),'color','k')
hpgB(2).LineWidth = 1.5;
ylim(ax(n),[-0.001,0.35])
ax(n).YGrid = 'on';
LpgB = legend(hpgB,'28 GHz OFF','28 GHz ON');
LpgB.Interpreter = 'latex';
LpgB.Box = 'off'; LpgB.Position = [0.5163    0.28    0.2141    0.0712];
LpgB.FontSize = 7;
