...
[RF,trf]   = my_mdsvalue_v2([shotlist(1)-1,shotlist(2)],DA(1));
% save('Optmizedfueling_17566.mat')
else
% load('Optmizedfueling_17566.mat')
end

Shotlist_Gas = [15927,13962,21269];
...
ax(n).YTick = [1:1:4];
ax(n).FontName = 'Times';
ylabel(ax(n),'$[\times 10^{19}$ $m^{-3}]$','FontSize',13); 
...
hne1(s) = plot(time{s}(rng),Ni{s}(rng)*1e-19,'Parent',ax(n),'color','k')
hne1(s).LineWidth = 1.5;

hne2(s) = plot(time{2},Ni{2}*1e-19,'Parent',ax(n),'color','bl');
hne2(s).LineWidth = 1.5;

rng = find(GlitchFlag{3}==0); nskip = 10;
hne3(s) = plot(time{3}(rng),Ni{3}(rng)*1e-19,'Parent',ax(n),'color','g');
hne3(s).LineWidth = 1.5;

Lne = legend([hne1(s),hne2(s),hne3(s)],'$Conf.$ 1','$Conf.$ 2','$Conf.$ 3');
...
ylim(ax(n),[0,5])
ax(n).YGrid = 'on';
text(ax(n),0.02,0.8,'Probe D','Units','normalized','Interpreter','latex','FontSize',10,'BackgroundColor','w')
line([tRF,tRF],[0,5],'color','k','linestyle',':','LineWidth',2)

% PG spool 6.5
n = 1;
ax(n).NextPlot = 'add';
% ax(n).YTick = [0.05,0.1,0.15,0.2, 0.25];
ax(n).FontName = 'Times';
ylabel(ax(n),'[Pa]','FontSize',11); %
ax(n).YLabel.Interpreter = 'latex';
ax(n).YLabel.Rotation = 90; 
ax(n).YLabel.HorizontalAlignment = 'center'; ax(n).YLabel.VerticalAlignment = 'bottom';
hpgB(1) = plot(TimeP-0.03,P{3}{1},'Parent',ax(n),'color','k')
hpgB(2) = plot(TimeP-0.03,P{3}{2},'Parent',ax(n),'color','bl')
hpgB(3) = plot(TimeP-0.03,P{3}{3},'Parent',ax(n),'color','g')

hpgB(1).LineWidth = 1.5;
hpgB(2).LineWidth = 1.5;
hpgB(3).LineWidth = 1.5;
