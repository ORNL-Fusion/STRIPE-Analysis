...
SweepType       = {    'niso'};
AttType         = {'Vx2,Ix5'};
Center_V        = [       1 ]; % Remove offset on V: 1 (yes) 0(no)
Center_I        = [       0 ]; % Remove offset on I: 1 (yes) 0(no)
tStart          = [     4.10]; % [s]
tEnd            = [     4.7 ];
...
% save('Optmizedfueling_17566.mat')
else
% load('Optmizedfueling_17566.mat')
end

Shotlist_Gas = [21269]; % Date 2018_04_13
% Shotlist_Gas = 21229; % Date 2018_04_13 no plasma
% Shotlist_Gas = 21226; % Date 2018_04_13 no plasma
% Shotlist_Gas = 21250; % Date 2018_04_13 no plasma
% Shotlist_Gas = 21739; % Date 2018_04_13 no plasma

% Shotlist_Gas = 17566; % Date 2017_11_10
% see 21229 for the no plasma case
% Shots 21269 and 17566 are almost equivalent, one has a good DLP data set
% while the other one has a good PG data set.

GasCompare_OptFueling;
...
ylim(ax(n),[0,120])
ax(n).YGrid = 'on';
text(ax(n),0.02,0.8,'RF power','Units','normalized','Interpreter','latex','FontSize',10)
...
hne(s).LineWidth = 1.5;
rng = [25:147,149:210];
hte10(s) = plot(time{s}(rng),Te{s}(rng),'Parent',ax(n),'color','bl');

Lne = legend([hne(s),hte10(s)],'$n_e{\times}10^{-19}$','$T_e$ $[eV]$');
...
ax(n).YLabel.HorizontalAlignment = 'center'; ax(n).YLabel.VerticalAlignment = 'bottom';
hpgB(1) = plot(TimeP-0.03,P{2}{1},'Parent',ax(n),'color','k')
...
ax(n).YLabel.Rotation = 90; 
ax(n).YLabel.HorizontalAlignment = 'center'; ax(n).YLabel.VerticalAlignment = 'bottom';
hpgB(1) = plot(TimeP-0.03,P{4}{1},'Parent',ax(n),'color','k')
% hpgB(2) = plot(tpg1{1}(1:end-1),PG1{1}/7.5,'Parent',ax(n),'color','k')
hpgB(1).LineWidth = 1.5;
ylim(ax(n),[-0.005,1.5])
ax(n).YGrid = 'on';
...
ax(n).YLabel.Rotation = 90; 
ax(n).YLabel.HorizontalAlignment = 'center'; ax(n).YLabel.VerticalAlignment = 'bottom';
hpgB(1) = plot(TimeP-0.03,P{3}{1},'Parent',ax(n),'color','r')
hpgB(1).LineWidth = 2;
ylim(ax(n),[-0.001,0.15])
ax(n).YGrid = 'on';
...
ax(n).YLabel.Rotation = 90; 
ax(n).YLabel.HorizontalAlignment = 'center'; ax(n).YLabel.VerticalAlignment = 'bottom';
hpgB(1) = plot(TimeP-0.03,P{1}{1},'Parent',ax(n),'color','k')
hpgB(1).LineWidth = 1.5;
ylim(ax(n),[-0.005,0.3])
ax(n).YGrid = 'on';
