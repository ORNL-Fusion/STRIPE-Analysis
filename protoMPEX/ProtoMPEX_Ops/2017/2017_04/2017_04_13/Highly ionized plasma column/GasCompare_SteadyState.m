...
shotlist = 21000 + [742,743,744,745,830,831];
dt       =         [0  ,100,200,300,400,0  ];
...
[PG1,t1] = my_mdsvalue_v2(Shotlist_Gas,address(1));
[PG2,t2] = my_mdsvalue_v2(Shotlist_Gas,address(2));
[PG3,t3] = my_mdsvalue_v2(Shotlist_Gas,address(3));
[PG4,t4] = my_mdsvalue_v2(Shotlist_Gas,address(4));
...
for s = 1 :length(Shotlist_Gas)
    P{1}{s} = ( PG1{s}(1:Ldata)-1*PG1cal{1}(1:Ldata) )*2*fctr;
    P{2}{s} = ( PG2{s}(1:Ldata)-1*PG2cal{1}(1:Ldata) )*2*fctr;
    P{3}{s} = ( PG3{s}(1:Ldata)-1*PG3cal{1}(1:Ldata) )*2*fctr;
    P{4}{s} = ( PG4{s}(1:Ldata)-1*PG4cal{1}(1:Ldata) )*10*fctr;
...
legend(h,{num2str(Shotlist_Gas')},'location','NorthWest')
set(gcf,'color','w')
