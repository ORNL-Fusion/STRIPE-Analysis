...
if CMPT == 1
    ...
    [PG,tpg] = my_mdsvalue_v2(shotlist,address(1));

    % B0 only calibration shots
    [PG0a,t0] = my_mdsvalue_v2([13469],address(1)); % 4000 A
    
    save('Shot_13781_82_NeTe.mat')
else 
    load('Shot_13781_82_NeTe.mat')
end
% note, 
...
for s = 1:length(shotlist)
    plot(tpg{s}(1:length(PG{s})),(PG{s}-PG0a{1})*f/7.5,C{s},'lineWidth',2)
end
ylim([0,0.5])
xlim([4.15,TimePlotEnd])
title('$ P_n $ $ [Pa] $','interpreter','Latex','FontSize',13,'Rotation',0)
set(findobj('-Property','YTick'),'box','on')
set(gcf,'color','w')

% 1 mTorr is 0.1333 Pa
% 1 Pa is 7.5 mTorr
if 1
    figure; hold on
    for s = 1:length(shotlist)
