% This code illustrates the use of the new DLP_V5_1 routine.
% This new version allows (1) centering of both V and I data and (2)
% provides additional information when routine fails to produce a fit due
% to "glitches" on the data.

% It based on data taken on 2017-04-28 where significant "glitches" were
% seen on the digitized signals.

% 1- Centering data:
% In order to get a good fit on shot 208, both V and I need to be centered.

% 2- Information on fitting error due to glitches on data:
% In order to circumvent fitting errors use tStart = 4.27, tEnd = 4.37
% To test new capabilities of the code use  tStart = 4.15, tEnd = 4.35
...
% NOTE on Center_V and Center_I:
% when using the isolated sweep, the raw signals are a few mV hence, any
% offset in the mV range caused by RF interference on the cables can lead
% to significant offsets on the V and I signals which can prevent the DLP
% code from finding a succesful fit.
