...
% It takes 22 sec to call up and plot all the data N = 22 shots
% it takes 3 sec to retrieve the above data if saved in a .mat file
% #########################################################################
...
shot = 13000 + [30 ,31 ,32 ,33 ,  34,  35,  36,  37, 38, 57, 58, 59, 60, 61, 62, 63, 64, 65, 66, 68, 69, 70, 71, 72, 73, 74,  75,  76,  77,  78,  79];
x    =         [8.0,8.5,9.0,9.5,10.0,10.5,11.0,11.5,7.5,7.5,8.5,8.0,7.5,7.0,6.5,6.0,5.5,5.0,4.5,4.0,4.5,5.5,3.5,3.0,2.5,8.5,11.0,11.5,12.0,12.5,13.0];
...
%         [PG4,t_pg4] = my_mdsvalue_v2([13006,13029],Address);
%         Address = {'\MPEX::TOP.MACHOPS1:PG3'};
%         [PG6,t_pg6] = my_mdsvalue_v2([13006,13029],Address);
%         % shot 13006: no RF, Gas + B0 only
%         % highest dPn occurs when bdot probe is fully retracted (13029)
%         
%         % Extract Ion sat of DLP6.5 for specific shots
%         Address = {'\MPEX::TOP.MACHOPS1:TARGET_LP'}; % Helicon signal
%         [Isat,t_isat] = my_mdsvalue_v2(13029,Address);
        
        save('Bz_TR1_0A_TR2_260A_data')
else
        load('Bz_TR1_0A_TR2_260A_data')
end
...
figure; hold on
for s = 1:length(shot)
    rng = find(tBdot{s}>=4.16 & tBdot{s}<=4.18);
    if 0
...
figure; hold on
for s = 1:length(shot)
    rng = find(tBdot{s}>=4.29 & tBdot{s}<=4.3);
    if 1
        Amplitude = sgolay_t(VmagRatio{s}(rng),3,61);
    else
        Amplitude = VmagRatio{s}(rng);
