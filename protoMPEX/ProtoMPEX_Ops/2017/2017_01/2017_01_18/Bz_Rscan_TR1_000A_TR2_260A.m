...
x =             [8.0,8.5,9  ,9.5,10 ,10.5, 11 ,11.5,12 ,12.5,13,13.5,7.5, 7 ,6.5, 6 ,5.5, 5 ,4.5, 4 ,3.5,3  ];
...
for ch = 1:4 % For all channels on digitizer
    ...
    title(Title{ch})
        ...
        save('Bz_TR1_000A_TR2_260A_data')
else
        load('Bz_TR1_000A_TR2_260A_data')
end
...
figure; hold on
for s = 1:length(shot)
    rng = find(tBdot{s}>=4.27 & tBdot{s}<=4.30);
    if 1
        Amplitude = sgolay_t(VmagRatio{s}(rng),3,61);
