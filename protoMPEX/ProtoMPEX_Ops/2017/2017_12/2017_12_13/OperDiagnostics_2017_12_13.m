...
shotlist = [18000 + [476,478]];
shotlist = [18000 + [429,435,438,440]];
% shotlist = [18000 + [482,483,484,485]];
% shotlist = [18000 + [429,492]];
...
for s = 1:length(shotlist)
    h(s) = plot(t_pgc{p}{s}(1:length(PGc{p}{s})),(0.133)*(PGc{p}{s}-RemoveOffset*min(PGc{p}{s})),C{s});
    set(h(s),'LineWidth',2)
    title(['PG ',T{p}])
    box on
    grid on
