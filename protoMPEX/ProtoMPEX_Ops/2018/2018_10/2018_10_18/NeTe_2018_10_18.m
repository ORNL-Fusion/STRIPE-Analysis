...
% 2018_10_18
% DLP 8.5 radial scan 
shotlist = [23000 + [620,621 ,622 ,624 ,625 ,626 ,627 ,628 ,629 ,630]];
R        =          [11 ,11.5,12.0,12.5,13.0,13.5,14.0,14.5,15.0,15.5];
% DLP 8.5 cleaning
% shotlist = [23000 + [629:633]];

% DLP 8.5 prior to ECH pulses
% shotlist = [23000 + [634,635]];
% shotlist = [23000 + [635]];

% DLP 8.5 with ECH pulses
shotlist = [23000 + [636,637]];
shotlist = [23000 + [640:642]];
shotlist = [23000 + [642:645]];
shotlist = [23000 + [661,655,662]];

% High ECH power radial scan DLP 8.5
shotlist = [23000 + [649,650,651 ,652,653,654,655,656 ,657,658 ,659,660 ]];
R =                 [11.5,11,10.5,10 ,9.5,9  ,12 ,12.5,13 ,13.5,14 ,14.5] ;

% Changed magnetic field to the uphill configuration
% Compare uphill to downhill configuration, 1 shot
shotlist = [23000 + [662,663]];
% shotlist = [23000 + [671,672,673,674,675]];
% shotlist = [23000 + [676,677]];

% DLP 8.5 Radial scan
% shotlist = [23000 + [663,664,665 ,666,667,668,669 ,670 ]];
% R        =          [12 ,11 ,10.5,10 ,9.5,9  ,11.5,12.5];

% On axis, low and high power shotsm uphill configuration
% shotlist = [23000 + [678,679,680]];

DLPType = '8MP';

% D2 gas puff at the target
% shotlist = [23000 + [688,689,690,691]];
% shotlist = [23000 + [693,694,695,696,697]];
shotlist = [23000 + [697,698,700]];
shotlist = [23000 + [700,703]];
% shotlist = [23000 + [701,702,703]];

% no Target shot
% shotlist = [23000 + [716,717,718]];
...
%         Config.L_tip = 1.6/1000;% according to Nischal 2017_11_21
    ...
    case '1MP'
        ...
        Config.L_tip = 3/1000; % As of 2018_09_19
        Config.D_tip = 0.254/1000; % [m]
    case 'TDLP'
        DLP = 11.5;
        Config.L_tip = 0/1000;
...
%     plot(time{s}(GoodFits{s}),ni{s}{2}(GoodFits{s}),C{s},'lineWidth',1);
    ...
    if s == 2
     ...
     plot(t_rf{s}(1:length(RF{s})),(RF{s}.^2)*30e19,C{s},'lineWidth',0.5)
