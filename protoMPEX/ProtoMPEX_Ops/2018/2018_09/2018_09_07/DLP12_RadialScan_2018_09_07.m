% The analysis in this script is not complete. all the correct shots have
% not been gathered.
...
DownloadData = 1;
SaveData = 1;
DataName = 'DLP12_RadiaScan_2018_09_07_RawData';
...
% =========================================================================
    ...
    MetaData.shot         =    22000 + [574 ,575 ,576 ,579  ,580 ,581 ,582  ,583 ,584  ,585 ,586  ,587 ,588  ,589 ,590  ,591  ,592 ,593   ,594  ,595  ,596   ,601];
    MetaData.r            =            [0.0 ,-0.5,-1.0,-0.25,0.25,0.5 ,0.75 ,1   ,1.25 ,1.5 ,1.75 ,2.0 ,2.25 ,2.5 ,1.25 ,-1.25,-1.5,-1.75 ,-2.0 ,-2.0 ,-2.25 ,0  ] ;
    MetaData.DataSetDescription     = ['DLP 12.5 radial scan with 28 GHz ECH to map out the drop in ne'];
    %--------------------------------------------------------------------------------------------------------------------------------------
    % Define ADDRESSES
    Address.V   =        {aV{1}};
    Address.I   =        {aI{1}};
...
for s = 1:length(Te)
    ...
    % ECH ON
    rng = find(time{s}>=4.49 & time{s}<=4.5);
    ne1(s) = mean(Ni{s}(rng));
...
h(1) = errorbar(RFVoltage,ne0,dne0)
h(1).Color = 'k'

h(2) = errorbar(RFVoltage,ne1,dne1)
h(2).Color = 'r'
...
h(3) = errorbar(RFVoltage,Te0,dTe0)
h(3).Color = 'k'

h(4) = errorbar(RFVoltage,Te1,dTe1)
h(4).Color = 'r'
ylim([0,7])
...
h(5) = errorbar(RFVoltage,e_c*ne0.*Te0,e_c*dne0.*dTe0)
h(5).Color = 'k'

h(6) = errorbar(RFVoltage,e_c*ne1.*Te1,e_c*dne1.*dTe1)
h(6).Color = 'r'
...
for s = 1:3
%     xlim(ax(s),[4,10]);
    xlabel(ax(s),'RF voltage','Interpreter','Latex','FontName','Times')
end
