...
% #########################################################################
% On Axis shot showing drop in ne on axis
shotlist = [14103,14107];
shotlist = [14057,14058];

% it is actually spool 10.5 but 4MP has the same dimensions
DLPType = '4MP';
...
for s = 1:length(shotlist)
    GoodFits{s} = GlitchFlag{s} == 0 & StdResNorm{s}<=0.4 & Ni{s}>0 & Ni{s}<20e19 & Te{s}<=20 & Is{s}>0.01e-3;
