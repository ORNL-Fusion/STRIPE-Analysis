...
for s = 1:length(shotlist)
    ...
    for p = 1:3
        ness{p}(s)  = mean(Ni{s}(rngT{p} & GoodFits{s}));
        dness{p}(s) = std(Ni{s}(rngT{p} & GoodFits{s}),1,2);
        Tess{p}(s)  = mean(Te{s}(rngT{p} & GoodFits{s}));
        dTess{p}(s) = std(Te{s}(rngT{p} & GoodFits{s}),1,2);
    end
end
...
hr(p) = errorbar(R(b)-11.25,ness{p}(b),dness{p}(b))
end
% hr(1).Color = 'k';
% hr(2).Color = 'r';
% hr(3).Color = 'g';
...
% Draw LFCS:
rDLP = 2.73;
line(+[1,1]*rDLP,[0,1e20],'LineWidth',2)
line(-[1,1]*rDLP,[0,1e20],'LineWidth',2)

ylim([0,10]*1e19)
xlim([-7.5,7.5]); 
set(gca,'XTick',[-7.5:1:7.5])
box on
grid on
...
for p = 1:3
ht(p) = errorbar(R(b)-11.5,Tess{p}(b),dTess{p}(b),'LineWidth',1)
end
% ht(1).Color = 'k';
% ht(2).Color = 'r';
% ht(3).Color = 'g';
...
% Sheath power profile at 200 kW
r_q = R(b) - 11.5;
TeLimiter = Tess{3}(b);
flux = 2*ness{3}(b).*C_s(TeLimiter,2);
q = 7*e_c*TeLimiter.*flux;
...
xlim([-7.5,7.5]); 
% Draw LFCS:
line(+[1,1]*rDLP,[0,10],'LineWidth',2)
line(-[1,1]*rDLP,[0,10],'LineWidth',2)
grid on
ylabel('Heat flux [MW]')
xlabel('r [m]')

% Total heat transported by hallo plasma
q_mean = 5.6e6; % [Wm^-2]
r1 = rDLP/100; % [m]
r2 = 4/100;    % [m]
A = pi*(r2^2 - r1^2);
...
FileName = 'NeTe_Spool_1_5_2018_01_16.xlsx';
xlswrite(FileName,[F;num2cell(G)]);
end