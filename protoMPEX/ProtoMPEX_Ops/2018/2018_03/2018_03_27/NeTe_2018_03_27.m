...
% 2018_03_27
% TR2 = 120 A
% shotlist = [20000 + [508,509]];
% R        =          [0.5,0,5] ;
% RFP      =          [46 ,89 ] ;

% TR2 = 180 A
shotlist = [20000 + [510,511]];
R        =          [0.5,0.5 ] ;
RFP      =          [91 ,91  ] ;

% TR2 = 180 A, MAGNETIZATION CURRENT
% shotlist = [20000 + [513]];
% R        =          [0.5 ] ;
% RFP      =          [91  ] ;

% % TR2 = 180 A,
% shotlist = [20000 + [511,514,515,516]];
% R        =          [0.5,0.5,0.5,0.5] ;
% RFP      =          [91 ,91 ,91 ,91 ] ;
% 
% % Gas flow scan
shotlist = [20000 + [516,517,518,519,520,522,523,524,525,526]];
R        =          [0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5] ;
RFP      =          [91 ,91 ,91 ,91 ,91 ,91 ,91 ,91 ,91 ,91 ] ;
GF       =          [3.0,3.5,4.0,4.5,5.0,5.5,6.0,7.0,3.0,2.5] ;

% Compare two power levels: 46 and 91 kW
shotlist = [20000 + [526,528]];
R        =          [0.5,0.5] ;
RFP      =          [91 ,46 ] ;
GF       =          [2.5,2.5] ;

% Retune to get high density mode at low gas level at 100 kW
shotlist = [20000 + [525,526,528,530,532,533,536]];
R        =          [0.5,0.5,0.5,0.5,0.5,0.5,0.5] ;
RFP      =          [91 ,91 ,46 ,62 ,90 ,90 ,90 ] ;
GF       =          [3.0,2.5,2.5,3.0,3.0,3.0,5.0] ;
TR2      =          [180,180,180,180,120,180,180] ;

% Dump tank On and Off
shotlist = [20000 + [536,537]];
R        =          [0.5,0.5] ;
RFP      =          [91 ,91 ] ;
GF       =          [3.0,3.0] ;
TR2      =          [180,180] ;
...
% Gas flow 5 V, radial scan with Dump tank OFF
shotlist = [20000 + [537,538,539,540,541,542]];
R        =          [0.5,1.0,1.5,2.0,2.5,3.0] ;
RFP      =          [91 ,91 ,91 ,91 ,91 ,91 ] ;
GF       =          [3.0,3.0,3.0,3.0,3.0] ;
TR2      =          [180,180,180,180,180] ;
DTankP   =          [0  ,0  ,0  ,0  ,0  ] ;

shotlist = [20000 + [536,543,544 ,546 ,547 ,548 ]];
R        =          [0.5,0.0,-0.5,-1.0,-1.5,-2.0] ;
RFP      =          [91 ,91 ,91  ,91  ,91  ,91 ] ;
GF       =          [3.0,3.0,3.0 ,3.0 ,3.0] ;
TR2      =          [180,180,180 ,180 ,180]  ;
DTankP   =          [0  ,0  ,0   ,0   ,0  ] ;   
% 
% Start of ECH experiments
shotlist = [20000 + [559, 560, 561, 562, 563]];
R        =          [0.5, 0.5, 0.5, 0.5, 0.5] ;
RFP      =          [45 , 63 , 90 , 90 , 90 ] ;
GF       =          [3.5,3.5 ,3.5 ,3.5 ,3.0 ] ;
TR2      =          [180,180 ,180 ,160 ,180 ] ;
DTankP   =          [0  ,0   ,0   ,0   ,0   ] ;
% 
% PS1 = 2kA
shotlist = [20000 + [564,567,568,569,570]];
% 
% Flux probe experiments at reduced Rf power
shotlist = [20000 + [525,625,626 ,627]];
R        =          [0.5,0.0,-0.5,-1.0,-1.5,-2.0] ;
RFP      =          [91 ,78 ,73  ,91  ,91  ,91 ] ;
GF       =          [3.0,3.0,3.0 ,3.0 ,3.0] ;
TR2      =          [180,180,180 ,180 ,180] ;
DTankP   =          [0  ,0  ,0   ,0   ,0  ] ;

% Radial scan at lower RF power
shotlist = [20000 + [626,627,628,629,630,631,632]];
R        =          [0.5,1.0,1.5,2.0,2.5,3.0,0.0] ;
RFP      =          [73 ,73 ,73 ,76  ,76  ,76 ] ;
GF       =          [3.0,3.0,3.0 ,3.0 ,3.0] ;
TR2      =          [180,180,180 ,180 ,180] ;
DTankP   =          [0  ,0  ,0   ,0   ,0  ] ;

% shotlist = [20000 + [626,632, 633, 634, 635, 636]];
% R        =          [0.5,0.0,-0.5,-1.0,-1.5,-2.0] ;
% RFP      =          [73 ,73 ,73  ,76  ,76  ,76 ] ;
% GF       =          [3.0,3.0,3.0 ,3.0 ,3.0] ;
% TR2      =          [180,180,180 ,180 ,180] ;
