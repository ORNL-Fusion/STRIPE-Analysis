...
% Examples of plasma production instead of Te increase:
% PS1 = 2kA
shotlist = [21000 + [305,306,308,309,311]];
% shotlist = [21000 + [309,312,313,314]];

% -------------------------------------------------------------------------
% Plasma production instead of electron heating:
shotlist = [21000 + [305,308,309]];

% Reducing the RF power lead to Te increase:
shotlist = [21000 + [313,312,305]];
RF       =          [  4,  5,  6] ;

% Plasma conditions finally re-established:
shotlist = [21000 + [324]];
% -------------------------------------------------------------------------
...
for s = 1:length(shotlist)
    GoodFits{s} = GlitchFlag{s} == 0 & StdResNorm{s}<=0.1 & Ni{s}>0 & Ni{s}<12e18 & Te{s}<=12 & Is{s}>0.01e-3;
