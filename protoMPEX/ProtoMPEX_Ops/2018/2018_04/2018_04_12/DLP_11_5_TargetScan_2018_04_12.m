% 2018_04_12, DLP 11.5 radial scan
% the probe used in this location was previoulsy located in spool 10.5, the
% tip lengths are 1.7 mm +- 0.5 mm
...
for s = 1:length(shotlist)
    GoodFits{s} = GlitchFlag{s} == 0 & StdResNorm{s}<=0.091 & Ni{s}>0 & Ni{s}<15e19 & Te{s}<=8 & Is{s}>1e-3;
...
for p = 1:3
        ...
        Pes{p}(s)    = mean(e_c*Ni{s}(rngT{p} & GoodFits{s}).*Te{s}(rngT{p} & GoodFits{s}));
        dPes{p}(s)   = std(e_c*Ni{s}(rngT{p} & GoodFits{s}).*Te{s}(rngT{p} & GoodFits{s}),1,2);
    end
end
...
for p = 3
y(p) = errorbar(Target_z(b1)-rOffset,nes{p}(b1),dnes{p}(b1),'LineWidth',1,'color','r','marker','o')
end
% set(y(1),'color','g')
% set(y(2),'color','r')
set(y(3),'color','k')
...
ylim([0,10]*1e19); ylabel('$n_e$ $[m^{-3}]$','Interpreter','Latex')
% xlim([-4,4]);
...
for p = 3
k(p) = errorbar(Target_z(b1)-rOffset,Tes{p}(b1),dTes{p}(b1),'LineWidth',1,'color','r','marker','o')
end
% set(k(1),'color','g')
% set(k(2),'color','r')
set(k(3),'color','k')
...
for p = 3
k(p) = errorbar(Target_z(b1)-rOffset,Pes{p}(b1),dPes{p}(b1),'LineWidth',1,'color','r','marker','o')
end
% set(k(1),'color','g')
% set(k(2),'color','r')
set(k(3),'color','k')
...
xlabel('$z$ $[cm]$','Interpreter','Latex')
% xlim([-4,4]);
ylabel('$P_e$ $[eV]$','Interpreter','Latex')
box on
grid on
% t = title('$P_e$ $[eV]$ '); t.Interpreter = 'latex'; t.FontSize = 13;
...
FileName = 'NeTe_Spool_9_5_2018_01_16.xlsx';
xlswrite(FileName,[F;num2cell(G)]);
end