...
for s = 1:length(shotlist)
    ...
    rngT{3} = [time{s}>=4.59 & time{s}<=4.63]; 
for p = 1:3
...
for p = 3
y(1) = errorbar(R(b1)-rOffset,nes{p}(b1),dnes{p}(b1),'LineWidth',1,'color','r','marker','o')
end
set(y(1),'color','k')
...
for p = 3
k(1) = errorbar(R(b1)-rOffset,Tes{p}(b1),dTes{p}(b1),'LineWidth',1,'color','r','marker','o')
end
set(k(1),'color','k')
