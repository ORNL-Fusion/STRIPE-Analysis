...
% =========================================================================
% 2018_04_06
shotlist = [20000 + [971,974,975,976,977]];

% RF power scan
shotlist = [20000 + [975:979,981,982]];
shotlist = [20000 + [982,983,984]];
shotlist = [20000 + [985,982]];
% shotlist = [20000 + [984,985,988,989,990,991]];
% % #####################################
% shotlist = [20000 + [991,993,994]];
% % #####################################
% 
% % RF power scan with ECH showing the effect of the UHR layer
% shotlist = [20000 + [991,995,996,997,998,999,1000,1002]];
% shotlist = [20000 + [991,998]];
% shotlist = [20000 + [991,998,1000,1002]];

% DLP 1.5 radial scan with ECH
shotlist = [21000 + [22  ,23  ,24 ,25 ,26 ,27 ,28 ,31  ,32  ,33  ,34  ,35  ]];
R        =          [10.5,10.0,9.5,9.0,8.5,8.0,7.5,8.75,10.5,11.0,11.5,12.0] ;
% 
% shotlist = [21000 + [36  ,38  ,39  ,40  ,41  ]];
% R        =          [12.5,13.0,13.5,14.0,14.5] ;

% DLP 1.5 radial scan with 28 GHz microwave power
shotlist = [21000 + [22  ,23  ,24 ,25 ,26 ,27 ,28 ,31  ,32  ,33  ,34  ,35  ,36  ,38  ,39  ,40  ,41  ]];
R        =          [10.5,10.0,9.5,9.0,8.5,8.0,7.5,8.75,10.5,11.0,11.5,12.0,12.5,13.0,13.5,14.0,14.5] ;
% 
% ECH power scan, clean IR, dlp 1.5 on-axis:
% shotlist = [21000 + [42]];
...
for s = 1:length(shotlist)
    GoodFits{s} = GlitchFlag{s} == 0 & StdResNorm{s}<=0.1 & Ni{s}>0 & Ni{s}<15e18 & Te{s}<=8 & Is{s}>0.1e-3;
...
for s = 1:length(shotlist)
    ...
    rngT{2} = [time{s}>=4.8 & time{s}<=4.82]; 
    rngT{3} = [time{s}>=4.66 & time{s}<=4.7]; 
...
ylim([0,2]*1e19); ylabel('$n_e$ $[m^{-3}]$','Interpreter','Latex')
