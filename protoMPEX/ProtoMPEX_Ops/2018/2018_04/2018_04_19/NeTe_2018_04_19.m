...
shotlist = [21000 + [346,350]];
% =========================================================================

% Back to the collisional heating experiments
shotlist = [21000 + [351,352,353]];
% Effec of puff on electron heating:
shotlist = [21000 + [351,359,360]];
Puff     =          [0  ,1  ,1  ];
% Effec of puff on helicon plasma only:
shotlist = [21000 + [362,363]];
% Comparing the effect gas injection with helicon and helicon + 28 GHz
shotlist = [21000 + [360,361,363]];

% % Puffing experiments
% shotlist = [21000 + [353,357,358,359,360]];
% % staggered injection
% shotlist = [21000 + [357,360,361,362]];
% % no ECH, D2 puff scan
% shotlist  = [21000 + [362,363,365,366]];
% PuffLevel =          [4.5,4.4,3.4,3.8] ;

% comparing w/ and w/o ECH, D2 puff 3.8v
shotlist  = [21000 + [366,367,369]];
PuffLevel =          [3.8,3.8,3.8] ;
ECH       =          [0  ,1  ,1  ] ;


% Neutral gas pressure at central chamber scan:
shotlist  = [21000 + [369,370,371,372,375 ]];
PuffLevel =          [3.8,4.0,3.9,0  ,3.85] ;
ECH       =          [1  ,1  ,1  ,1  ,1   ] ;

% Compare no gas case with lowest gas case
shotlist  = [21000 + [380 ,370,371,369 ,372]];
PuffLevel =          [4.15,4.0,3.9,3.85,0  ] ;
ECH       =          [1   ,1  ,1  ,1  ,1   ] ;

% Observing response w/ and w/o microwave injection
% collisional heating does not induce reduction in plasma flux like during
% the ECH/EBW heating experiments
shotlist  = [21000 + [357 ,372]];
shotlist  = [21000 + [411]];
...
% =========================================================================
% Radial scan with D2 puff
shotlist  = [21000 + [380,381,382,383,384,385,386,387,388]];
R         =          [6.5,7.0,7.5,8.0,8.5,9.0,9.5,6.0,5.5] ;

shotlist  = [21000 + [387,388,389,390,391,392,393]];
R         =          [6.0,5.5,5.0,4.5,4.0,3.5,3.0] ;
