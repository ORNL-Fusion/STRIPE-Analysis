...
switch ProbeLoc
    case 'A'
% #########################################################################
shotlist = 21000 + [333 ,411 ,432 ,433 ,480 ]; 
date     =         [0419,0419,0420,0420,0425];

shotlist = 21000 + [546 ,548 ,597 ,630 ,663 ,683 ,701 ]; 
date     =         [0426,0426,0426,0427,0427,0427,0427];

shotlist = 21000 + [713 ,717 ]; 
date     =         [0430,0430];

shotlist = 21000 + [741 ,746 , 747, 750 ,809, 810 ]; 
date     =         [0501,0501];

shotlist = 21000 + [746 , 747 ]; 


% shotlist = [21350,21630, 21683,21714,21727];
...
switch AttType
    ...
    case 'Vx0_5,Ix1'
        Config.V_Att = 0.5;  % Output voltage of DLP box (Voltage) = V_Att*Digitized data 
        Config.I_Att = 1;  % Output voltage of DLP box (Current) = I_att*Digitized data
end
% #########################################################################
...
for s = 1:length(shotlist)
    GoodFits{s} = GlitchFlag{s} == 0 & StdResNorm{s}<=0.22 & Ni{s}>0 & Ni{s}<12e19 & Te{s}<=12 & Is{s}>0.01e-3;
