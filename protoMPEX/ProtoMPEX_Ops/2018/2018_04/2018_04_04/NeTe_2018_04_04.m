...
for s = 1:length(shotlist)
    GoodFits{s} = GlitchFlag{s} == 0 & StdResNorm{s}<=0.15 & Ni{s}>0 & Ni{s}<1e21 & Te{s}<=8 & Is{s}>7e-3;
...
%     plot(time{s}(GoodFits{s}),ni{s}{2}(GoodFits{s}),C{s},'lineWidth',1);
    ...
    plot(t_rf{s}(1:length(RF{s})),(RF{s}.^2)*5e19,C{s},'lineWidth',0.5)
...
for p = 3
h(1) = errorbar(R(rngECH_ON(b1))-rOffset,nes{p}(rngECH_ON(b1)),dnes{p}(rngECH_ON(b1)),'LineWidth',1,'color','r','marker','o')
h(2) = errorbar(R(rngECH_OFF(b0))-rOffset,nes{p}(rngECH_OFF(b0)),dnes{p}(rngECH_OFF(b0)),'LineWidth',1,'color','k')
end
legend([h(1),h(2)],'28 GHz ON','28 GHz OFF')
...
xlim([-3,3]); xlabel('$R$ $[cm]$','Interpreter','Latex')
box on
grid on
...
for p = 3
h(1) = errorbar(R(rngECH_ON(b1))-rOffset,Tes{p}(rngECH_ON(b1)),dTes{p}(rngECH_ON(b1)),'LineWidth',1,'color','r','marker','o')
h(2) = errorbar(R(rngECH_OFF(b0))-rOffset,Tes{p}(rngECH_OFF(b0)),dTes{p}(rngECH_OFF(b0)),'LineWidth',1,'color','k')
end
legend([h(1),h(2)],'28 GHz ON','28 GHz OFF','location','south')
ylim([0,7]); xlabel('$R$ $[cm]$','Interpreter','Latex')
xlim([-3,3]); ylabel('$T_e$ $[eV]$','Interpreter','Latex')
box on
grid on
...
hold on
h(2) = plot(t_Ni_gf{n1},Ni_gf{n1},'r','lineWidth',2);
h(1) = plot(t_Ni_gf{n0},Ni_gf{n0},'k','lineWidth',2);
...
title('DLP 10.5 on-axis')
hold on
h(2) = plot(t_Te_gf{n1},Te_gf{n1},'r','lineWidth',2);
h(1) = plot(t_Te_gf{n0},Te_gf{n0},'k','lineWidth',2);
legend([h(1),h(2)],'28 GHz OFF','28 GHz ON','location','northeast')
