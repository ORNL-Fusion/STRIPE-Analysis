...
shotlist = [13971,13969,21269,21226,21230];
shotlist = [21269,21226];

close all
TR2_filt =         ones(size(shotlist));
TR2_ffrm =         ones(size(shotlist))*15;
Calshot  =         ones(size(shotlist))*14120;
PGcenter =         [];
...
for s = 1:length(shotlist)
    rng0 = find(t_pgc{p}{s}>=3.8 & t_pgc{p}{s}<=4);
    h(s) = plot(t_pgc{p}{s}(1:length(PGc{p}{s})),PGc{p}{s}-RemoveOffset*mean(PGc{p}{s}(rng0)),C{s});
    set(h(s),'LineWidth',2)
    title(['PG ',T{p}])
    box on
    grid on
