...
shotlist = 19800 + [107];
shotlist = 19900 + [59];
shotlist = 20000 + [52:56];
shotlist = 20000 + [57:61];
shotlist = 20000 + [55,57];
shotlist = 20000 + [57:58];
shotlist = 20000 + [61,62];
shotlist = 20000 + [81,82];
shotlist = 20000 + [83];
shotlist = 20000 + [203];
...
for s = 1:length(shotlist)
    plot(t_f1{s}(1:end-1),(f1{s}-0*min(f1{s}))*20,C1{s},'LineWidth',2)
    plot(t_f2{s}(1:end-1),(f2{s}-0*min(f2{s}))*20,C2{s},'LineWidth',2)
    plot(t_f3{s}(1:end-1),(f3{s}-0*min(f3{s}))*20,C3{s},'LineWidth',2)
    plot(t_f4{s}(1:end-1),(f4{s}-0*min(f4{s}))*20,C4{s},'LineWidth',2)
end
