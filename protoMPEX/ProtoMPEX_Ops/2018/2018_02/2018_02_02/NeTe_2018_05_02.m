...
% =========================================================================
shotlist = 19515;
% shotlist = [19500 + [45,46,47,49,50,51]];
% shotlist = [19500 + [45,46]]; % 5100 A
% shotlist = [19500 + [50,51]]; % 5600 A
% shotlist = [19500 + [47,49]]; % 6100 A
% 
% % Radial scan with ECH
% shotlist = [19500 + [30 ,31 ,  32, 34 ,  35]]; % Radial scan at 5600 A
% R        =          [0.5,0.0,-0.5,-1.0,-1.5]
% 
% shotlist = [19500 + [35 ,34 ,  32, 31 ,  30]]; % Radial scan at 5600 A
% R        =          [0.5,0.0,-0.5,-1.0,-1.5]

% Radial no helion plasma, ECH on
% shotlist = [19500 + [39:41]]; % 
...
for s = 1:length(shotlist)
    GoodFits{s} = GlitchFlag{s} == 0 & StdResNorm{s}<=0.1 & Ni{s}>0 & Ni{s}<1e21 & Te{s}<=30;
...
for s = 1:length(shotlist)
    f = sgolay_t(Te{s}(GoodFits{s}),3,7);
    h(s) = plot(time{s}(GoodFits{s}),f,C{s},'lineWidth',2)
