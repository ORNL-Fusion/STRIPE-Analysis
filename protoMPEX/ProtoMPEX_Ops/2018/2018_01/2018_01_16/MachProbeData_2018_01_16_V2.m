...
Rad      =         [4.5 ,4.5 ,4.2 ,3.9 ,3.6 ,3.3 ,3.0 ,2.7 ,2.4 ,2.1 ,1.8 ];
...
%     xlim([1,4]*1e4)
    ...
    rngSS = find(RisingEdgeMaxLocs{s}>=find(t2{s}>4.48,1) & RisingEdgeMaxLocs{s}<=find(t2{s}>4.55,1));
...
figure;
subplot(2,1,1)
errorbar(Rad,MachNumberSS,1*dMachNumberSS,'r-','LineWidth',2)
xlim([0,5])
ylim([0,1])
grid on
set(gcf,'color','w')
