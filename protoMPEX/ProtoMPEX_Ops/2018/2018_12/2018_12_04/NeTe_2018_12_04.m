...
% #########################################################################
% 2018_12_04
shotlist = 24000 + [700,723,724,728,729,730];
shotlist = 24000 + [700,733,759];
% Effect of ICH on DLP 6.5:
shotlist = 24000 + [735,736];
shotlist = 24000 + [740,742];
shotlist = 24000 + [770,771];

% DLP 6.5 radial scan with ICH
% shotlist = 24000 + [735,737,738 ,740 ,741];
% R        =         [11 ,13 ,12.5,10.5,10 ];
% shotlist = 24000 + [771,772,769,761,762,763,764,765];
% PWR_ICH  =         [  0,  9,12 ,12 ,15 ,19 ,22 ,28 ];
% shotlist = 24000 + [761,762,764,767,771];
% PWR_ICH  =         [ 12,  16,22,30 , 0 ];
DLPType = '6MP';

% shotlist = 24000 + [773,774,775,776,777,778,779];
% PWR_ICH  =         [ -2,  0,  1,  2,  3,  4,  5];
% PWR_ICH  =         [9  , 10, 16, 20,23 ,28 , 33];
shotlist = 24000 + [775,777,778,779,782];
PWR_ICH  =         [16 ,23 ,28 , 33,0  ];
% shotlist = 24000 + [774,777,779];
DLPType = '12MP';

% Radial scan 12.5
% shotlist = 24000 + [781,782 ,783 ,784 ,786,787,788,789];
% R        =         [9.5,10.0,10.0,10.5,9.5,9.0,8.5,8.0];
% DLPType = '12MP';
% 
% shotlist = 24000 + [786,790];
% DLPType = '12MP';
% #########################################################################
...
for s = 1:length(shotlist)
    GoodFits{s} = GlitchFlag{s} == 0 & StdResNorm{s}<=0.4 & Ni{s}>0 & Ni{s}<30e19 & Te{s}<=20 & Is{s}>0.01e-3;
...
%     plot(t_ech{s}(1:length(ECH{s})),abs(ECH{s})*1e19,C{s},'lineWidth',0.5)
     ...
     plot(t_ICH_F{s}(1:length(ICH_F{s})),(ICH_F{s}.^2)*5e19,C{s},'lineWidth',1.5)
...
for s = 1:length(shotlist)
    rngT{1} = [time{s}>=4.3 & time{s}<=4.32]; 
    rngT{2} = [time{s}>=4.4 & time{s}<=4.5]; 
    rngT{3} = [time{s}>=4.58 & time{s}<=4.62]; 
for p = 1:3
...
if 1
    ...
    n = 2; errorbar(R,nes{n},dnes{n},'ro')
    n = 3; errorbar(R,nes{n},dnes{n},'ko')
    ylim([0,5e19])
    
    subplot(2,1,2); hold on
    n = 2; errorbar(R,Tes{n},dTes{n},'ro')
    n = 3; errorbar(R,Tes{n},dTes{n},'ko')
    ylim([0,6])
end
