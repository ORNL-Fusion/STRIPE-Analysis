...
shotlist = 24000 + [771,765];
Rad  =             [  0,28 ];

% DLP 6.5 
shotlist = 24000 + [740];
shotlist = 24000 + [770,771];
shotlist = 24000 + [761,762,764,767];
% shotlist = 24000 + [769,770];
...
%     Select data range:
    MPI_1{s} = sgolay_t(f_1{s}(rng),3,13);
    MPI_2{s} = sgolay_t(f_2{s}(rng),3,13);
    t1{s} = tf1{1}(rng);
    t2{s} = tf2{1}(rng);
...
%     title(['R= ',num2str(x(s))])
    ...
    if S1 > S2
            ...
            dt = t2{s}(2)-t1{s}(1);
            dn = floor(0.8*T/dt);
            [r,~] = peakseek(h1,dn,0); % zero crossing, minpeadist, minpkht
...
for s = 1:length(shotlist)
    ...
    hold on
    plot(t1{s}(RisingEdgeMaxLocs{s}),J1{s}*1000/R,'k.-')
    plot(t2{s}(RisingEdgeMaxLocs{s}),J2{s}*1000/R,'r.-')
    plot(trf{s}(1:end-1),5*10.^(RF{s}),'k')
...
for s = 1:length(shotlist)
    ...
    plot(t1{s}(RisingEdgeMaxLocs{s}),MachNumber{s},'k.-')
    plot(trf{s}(1:end-1),0.2*(10.^(RF{s}) - 1.3),'r')
