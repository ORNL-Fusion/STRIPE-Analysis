...
% #########################################################################
% 2018_11_29
shotlist = 24000 + [600,601,602];
shotlist = 24000 + [603,605,607,608];
shotlist = 24000 + [610,611];
DLPType = '12MP';
% 
shotlist = 24000 + [616,617,618,619,621];
shotlist = 24000 + [613,615,617,618,620,621,623,627];
...
for s = 1:length(shotlist)
    GoodFits{s} = GlitchFlag{s} == 0 & StdResNorm{s}<=0.2 & Ni{s}>0 & Ni{s}<30e19 & Te{s}<=20 & Is{s}>0.01e-3;
...
%     plot(t_ech{s}(1:length(ECH{s})),abs(ECH{s})*1e19,C{s},'lineWidth',0.5)
     ...
     plot(t_rf{s}(1:length(RF{s})),(RF{s}.^2)*5e19,C{s},'lineWidth',1.5)
     plot(t_ICH_F{s}(1:length(ICH_F{s})),(ICH_F{s}.^2)*30e19,C{s},'lineWidth',1.5)
     plot(t_ICH_R{s}(1:length(ICH_R{s})),(ICH_R{s}.^2)*30e19,C{s},'lineWidth',1.5)
