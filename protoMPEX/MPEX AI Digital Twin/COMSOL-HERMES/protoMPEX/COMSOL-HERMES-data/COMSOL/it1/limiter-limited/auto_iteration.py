...
def runComsol(wkdir,comsolFile):
 commandList=['comsol','batch','-inputfile',comsolFile,'-study','std1'];
 print(commandList)
 print(wkdir)
 subprocess.run(commandList,cwd=wkdir)
...
def doCase(caseDir):
 ...
 for i,nextDir in enumerate(nextDirs):
  ...
  # run comsol again
  runComsol(nextDir,'loop_it'+str(i+1)+'.mph')
  # check if output files exist
