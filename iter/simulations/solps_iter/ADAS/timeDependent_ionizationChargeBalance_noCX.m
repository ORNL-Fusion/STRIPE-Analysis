...
%% 3) Compute S and R Arrays via Interpolation (no CX)
...
%% 5) Solve the ODE System for Ionization Balance (No CX)
...
[tSol, fSol] = ode23s(@(t, f) ionBalanceODE_noCX(t, f, S, R, ne, Zmax), ...
                      tspan, fSS, opts);
...
title('Neon Ionization Balance (No CX, Initial = Coronal fSS)');
...
%% ODE FUNCTION: Ionization Balance (No CX)
function dfdt = ionBalanceODE_noCX(~, f, S, R, ne, Zmax)
    % f: (Zmax+1)x1 vector, f(z+1) = fraction at charge state z
    % S, R: length (Zmax+1) arrays with S(z+1)=S_z, R(z+1)=R_z
    % ne: electron density

    dfdt = zeros(Zmax+1, 1);

    % z = 0 (i = 1)
    dfdt(1) = - ne * S(1) * f(1) + ne * R(2) * f(2);  % Ion loss + Recomb gain

    % z = 1 .. Zmax-1 (i = 2 .. Zmax)
    for i = 2:Zmax
        dfdt(i) = ne * S(i) * f(i-1) ...                   % Ion gain from (z−1)
                  - ne * ( S(i+1) + R(i) ) * f(i) ...       % Ion loss + Recomb loss
                  + ne * R(i+1) * f(i+1);                   % Recomb gain from (z+1)
    end

    % z = Zmax (i = Zmax+1)
    i = Zmax + 1;
    dfdt(i) = ne * S(i-1) * f(i-1) ...  % Ion gain from Zmax−1 → Zmax
