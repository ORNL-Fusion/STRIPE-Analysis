...
%=====================================================================
% Neon Ionization Balance with Charge Exchange (All Fully‐Stripped Initial)
%---------------------------------------------------------------------
% This script:
%  1) Loads ADAS rate coefficients from a NetCDF file
%  2) Interpolates S, R, CX at the given Te, ne
%  3) Computes a self‐consistent equilibrium fSS (for reference)
%  4) Solves the time‐dependent ODE for f_z including charge exchange (CX)
%     but starts with *all ions fully stripped* (f₁₀ = 1 for Ne^10+)
...
parfor j = 1:Zmax
    ...
    % Prevent zeros (avoid divide-by-zero in equilibrium calc)
    S(j)    = max(S(j),    eps);
    R(j+1)  = max(R(j+1),  eps);
...
%% 4) Compute Self‐Consistent Equilibrium fSS (for Reference Only)
fSS = zeros(Zmax+1, 1);
fSS(1) = 1;  % Start with all neutrals (z=0) for reference
...
disp('✅ Equilibrium initial charge distribution fSS (for comparison):');
disp(fSS);

%% 5) Solve the ODE System for Ionization Balance with CX
% Initial condition: all fully‐stripped (Ne^{10+})
f0 = zeros(Zmax+1, 1);
f0(end) = 1;  % f0(11) = 1 → 100% Ne^{10+}
...
%% 7) Plot Time Evolution & Inset: Final vs Equilibrium Initial
figure;
hold on;
colorMap = jet(Zmax+1);
for i = 1:(Zmax+1)
...
title('Neon Ionization Balance with CX (All Fully‐Stripped Initially)');
legend(arrayfun(@(z) sprintf('Ne^{%d+}', z), 0:Zmax, 'UniformOutput', false), 'Location', 'best');
set(gca, 'XScale', 'log');
grid on;
hold off;

% Inset axes: compare final distribution vs equilibrium reference
...
% Line plot: equilibrium initial distribution (fSS)
...
legend(insetAxes, 'Final Ionization State', 'Equilibrium fSS', 'Location', 'best');
grid(insetAxes, 'on');

% Color each bar according to charge state
for i = 1:(Zmax+1)
...
function dfdt = ionBalanceODE_CX(~, f, S, R, CX, ne, n_n, Zmax)
    % f: (Zmax+1)x1 vector, f(z+1) = fraction at charge z
