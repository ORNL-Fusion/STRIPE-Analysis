close all
clear all
[v, f, n, name] = stlRead('IterGeom_GITR.stl');
C = f;
p = v;
...
% limiter_inds = find((centroid(:,1) < 8500) & (centroid(:,2)>700) & centroid(:,3) > -438 & centroid(:,3) < 1678);
...
patch(transpose(X),transpose(Y),transpose(Z),ne,'FaceAlpha',1,'EdgeAlpha', 0.3 )

figure; 
patch(transpose(X),transpose(Y),transpose(Z),e_value(plotSet),'FaceAlpha',1,'EdgeAlpha', 0.3 )

% figure;
% [planes_lim tet_cen_lim] = get_triangles_from_stl('ITER_geom.stl','b',0.1,1)
...
hold on;
scatter3(tet_centers(:,1),tet_centers(:,2),tet_centers(:,3),'red')
...
% on lines 138-143
materialZ = 74*zeros(length(planes),1);
...
patch(transpose(X),transpose(Y),transpose(Z),'b','FaceAlpha',0.5,'EdgeColor','k')%'none');
scatter3(centroid(:,1),centroid(:,2),centroid(:,3),'cyan')
figure;
patch(transpose(X),transpose(Y),transpose(Z),'b','FaceAlpha',0.5,'EdgeColor','k'); hold on;

plot3([tet_centers(:,1), centroid(:,1)]', [tet_centers(:,2), centroid(:,2)]',[tet_centers(:,3), centroid(:,3)]' )
...
figure(402)
patch(transpose(X),transpose(Y),transpose(Z),dot_prod4selectSurf,'FaceAlpha',1,'EdgeColor','k')%'none')
title('GITR non-surfaces: End-caps are absorbing boundaries')


plotSet = find(surfs) %:length(planes) ; %find(surfs==0);
X = [planes((plotSet),1),planes((plotSet),4),planes((plotSet),7)];
Y = [planes((plotSet),2),planes((plotSet),5),planes((plotSet),8)];
Z = [planes((plotSet),3),planes((plotSet),6),planes((plotSet),9)];
...
% planes = [X(:,1) Y(:,1) Z(:,1) X(:,2) Y(:,2) Z(:,2) X(:,3) Y(:,3) Z(:,3)];
figure(401)
patch(transpose(X),transpose(Y),transpose(Z),'b','FaceAlpha',1,'EdgeColor','k')%'none')
title('GITR non-surfaces: End-caps are absorbing boundaries')


writematrix(norm_vec,"norm_vec.csv");

plotSet = 100:150 %:length(planes) ; %find(surfs==0);
X = [planes((plotSet),1),planes((plotSet),4),planes((plotSet),7)];
Y = [planes((plotSet),2),planes((plotSet),5),planes((plotSet),8)];
Z = [planes((plotSet),3),planes((plotSet),6),planes((plotSet),9)];
...
plotSet = [1:10,150:length(planes)] ; %find(surfs==0);
X = [planes((plotSet),1),planes((plotSet),4),planes((plotSet),7)];
Y = [planes((plotSet),2),planes((plotSet),5),planes((plotSet),8)];
Z = [planes((plotSet),3),planes((plotSet),6),planes((plotSet),9)];
hold on;
patch(transpose(X),transpose(Y),transpose(Z),'g','FaceAlpha',1,'EdgeColor','k')%'none')


potential=ones(1,length(planes))*100; % Initialize potential=1000 V
format = '%.16e';
...
ne=load('ne.csv');
% ne_surface=ne(100,:);
% figure; pcolor(r,z,ne)



r_centroid = sqrt(centroid(:,1).^2 + centroid(:,2).^2);
z_centroid =  centroid(:,3);
% figure;scatter(r_centroid,z_centroid); 

ne_surface = interpn(r,z,ne',r_centroid,z_centroid );

Y0 = 0.1;
eroded_flux= readmatrix('eroded_flux.csv'); % proportional to density now, need some realistic values
erosion = [eroded_flux;0].*area;
writematrix(erosion,'erosion_rate.csv');
