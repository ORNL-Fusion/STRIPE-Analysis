...
hold on
limiter_inds = find((centroid(:,1) < 8500) & (abs(centroid(:,2))>800))
plotSet= limiter_inds;
X = [planes((plotSet),1),planes((plotSet),4),planes((plotSet),7)];
Y = [planes((plotSet),2),planes((plotSet),5),planes((plotSet),8)];
Z = [planes((plotSet),3),planes((plotSet),6),planes((plotSet),9)];
figure
patch(transpose(X),transpose(Y),transpose(Z),'r','FaceAlpha',0.5,'EdgeColor','k')
hold on;
ne=readmatrix('iter_den.csv');
patch(transpose(X),transpose(Y),transpose(Z),ne(limiter_inds),'FaceAlpha',1,'EdgeAlpha', 0.3 )
