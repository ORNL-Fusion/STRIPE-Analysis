...
% importGeometry(model,'west.stl');% Import STL file
figure(2)
...
for i=1:length(centroid)
    ...
    potential_surf(i) = emag(I);
     ...
     if centroid(i,3) < -0.4198 && centroid(i,3) > 1.6599 %#ok<ALIGN>
         potential_surf(i) = 0;
end
...
% surface_variables=table(potential_surf,ne_surf,te_surf,ti_surf,v_mag,b_mag,br_surf,bt_surf,bz_surf,theta,ni_surf);
...
figure;
patch(transpose(X),transpose(Y),transpose(Z),ne_surf(plotSet),'FaceAlpha',0.3,'EdgeAlpha', 0.3 );
...
figure; 
patch(transpose(X),transpose(Y),transpose(Z),potential_surf(plotSet),'FaceAlpha',1,'EdgeAlpha', 0.1 )
