close all;
clearvars -global;
load("extrapolatedData_solps_ITER.mat");
% load("extrapolatedData_solps_ITER_masked.mat");
...
% Example: core defined as central circular region (adjust as needed)
R = sqrt((X - mean(X(:))).^2 + (Y - mean(Y(:))).^2);
core_radius = 0.05;
core_mask = R < core_radius;  % e.g., core_radius = 0.05 or something

for i = 1:length(vars2D)
    vars2D{i}(core_mask) = NaN;  % Insert NaNs in core
end
varsToPlot = {val_ne, val_Te, val_gradTi, val_vr, val_vz, val_vt};
titles = {'Electron Density (neS)', 'Electron Temperature (TeS)', ...
          'Ion Temperature Gradient (gradTiS)', ...
...
nR = length(x);
nZ = length(z);
ncid = netcdf.create('profilesITER.nc','NC_WRITE');
