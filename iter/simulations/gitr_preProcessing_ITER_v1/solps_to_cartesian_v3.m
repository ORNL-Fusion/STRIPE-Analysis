...
te_cv = double(State.te(:))./1.602e-19;     % [Nc x 1]
ti_cv = double(State.ti(:))./1.602e-19;     % [Nc x 1]
...
%% === EXTRA MIDPLANE CHECK: Te vs Ti (SOLPS & GITR) ===
figure('Color','w','Position',[120 120 1100 450]);
tiledlayout(1,2,'TileSpacing','compact','Padding','compact');

nexttile(1);
plot(rgrid, Te_SOLPS_mid, 'r-', 'LineWidth', 2); hold on;
plot(rgrid, Ti_SOLPS_mid, 'b-', 'LineWidth', 2);
plot(Rcheck, Te_GITR_mid, 'r--', 'LineWidth', 2);
plot(Rcheck, Ti_GITR_mid, 'b--', 'LineWidth', 2);
xlabel('R [m]'); ylabel('Temperature [eV]');
title('Midplane Te and Ti');
legend('Te SOLPS','Ti SOLPS','Te GITR','Ti GITR','Location','best');
grid on; box on;

nexttile(2);
ratio_SOLPS = Te_SOLPS_mid ./ max(Ti_SOLPS_mid, 1e-30);
ratio_GITR  = Te_GITR_mid  ./ max(Ti_GITR_mid,  1e-30);
plot(rgrid, ratio_SOLPS, 'k-', 'LineWidth', 2); hold on;
plot(Rcheck, ratio_GITR, 'm--', 'LineWidth', 2);
xlabel('R [m]'); ylabel('T_e / T_i');
title('Midplane Te/Ti Ratio');
legend('SOLPS','GITR','Location','best');
grid on; box on;

fprintf('Mean |Δ(Te/Ti)|: %.3e\n', ...
    nanmean(abs(ratio_SOLPS - interp1(Rcheck,ratio_GITR,rgrid,'linear','extrap'))));
