...
nR = 4000; nZ = 8000;                              % keep your current res
...
%% === WRITE NETCDF (singleSpecies OR multiSpecies) ==========================
type = 'singleSpecies';   % <== SWITCH here: 'singleSpecies' or 'multiSpecies'

switch type

    %% ----------------------------------------------------------------------
    case 'singleSpecies'
        disp('Writing singleSpecies NetCDF (D-only / single-fluid)...');
        ...
        % Define plasma profile variables
        vid_ne  = netcdf.defVar(ncid, 'ne',  'double', [dimR dimZ]);
        vid_ni  = netcdf.defVar(ncid, 'ni',  'double', [dimR dimZ]);
        ...
        % Write single-fluid quantities (transpose to [nR x nZ])
        netcdf.putVar(ncid, vid_ne, ne_q');
        netcdf.putVar(ncid, vid_ni, ne_q');         % assume quasi-neutral
        netcdf.putVar(ncid, vid_te, Te_q');
        netcdf.putVar(ncid, vid_ti, Ti_q');
        netcdf.putVar(ncid, vid_vr, uR_all(:,:,2)'); % using D+ or e− as proxy
        netcdf.putVar(ncid, vid_vt, uT_all(:,:,2)');
        netcdf.putVar(ncid, vid_vz, uZ_all(:,:,2)');
        % netcdf.putVar(ncid, vid_psi, psiN');
        ...
        netcdf.close(ncid);
        fprintf('✅ Wrote %s (singleSpecies, D-only)\n', outnc);

    %% ----------------------------------------------------------------------
    case 'multiSpecies'
        disp('Writing multiSpecies NetCDF (D + Ne + B + velocities)...');
        outnc = fullfile(pwd, 'profiles_iter_multi.nc');
        if exist(outnc,'file'), delete(outnc); end
        ...
        % Write coordinates/meta
        netcdf.putVar(ncid, vid_x, rgrid);
        netcdf.putVar(ncid, vid_z, zgrid);
        netcdf.putVar(ncid, vid_Z, zn);
        netcdf.putVar(ncid, vid_q, zn);   % or explicit charge array if available
        ...
        % Write B-fields (transpose to [nR x nZ])
        netcdf.putVar(ncid, vid_br, Br_q');
        netcdf.putVar(ncid, vid_bt, Bt_q');
        netcdf.putVar(ncid, vid_bz, Bz_q');

        % Write 3D species data ([nZ x nR x ns] → [nR x nZ x ns])
    ...
    %% ----------------------------------------------------------------------
    otherwise
        error('Unknown export type "%s". Choose "singleSpecies" or "multiSpecies".', type);
end
%% === QUICK PLOTS (sanity) ===
