...
data=readmatrix('Targets_Ne6+.txt');
yields_data=readmatrix('yields_Ne6+.csv');
potential_data=data(:,1);
ne_data=data(:,11);
te_data=data(:,3);
v_data=data(:,5);
ero_data=[0;yields_data(:,6)].*ne_data.*v_data;
...
patch(transpose(X),transpose(Y),transpose(Z),[0;yields_data(:,6)],'FaceAlpha',1,'EdgeAlpha', 0.3)%,impacts(surface)
title('Yields')
