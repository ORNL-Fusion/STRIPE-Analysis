...
%% ======================= EFFECTIVE YIELDS (LINEAR SCALE) =====================
% Ion flux used for weighting
ion_flux = all_dens .* all_flow;   % Nfaces x Nspecies

% ---------- D+ effective yield ----------
num_D = all_yield(:,1) .* ion_flux(:,1);
den_D = ion_flux(:,1);

effYield_D = nan(Nfaces,1);
maskD = den_D ~= 0;
effYield_D(maskD) = num_D(maskD) ./ den_D(maskD);
effYield_D(~maskD) = all_yield(~maskD,1);  % fallback if zero flux

% ---------- Neon (all charge states) effective yield ----------
ne_idx = 2:size(all_yield,2);   % Ne1+ ... Ne10+
num_Ne = sum(all_yield(:,ne_idx) .* ion_flux(:,ne_idx), 2, 'omitnan');
den_Ne = sum(ion_flux(:,ne_idx), 2, 'omitnan');

effYield_Ne = nan(Nfaces,1);
maskNe = den_Ne ~= 0;
effYield_Ne(maskNe) = num_Ne(maskNe) ./ den_Ne(maskNe);
% zero-Ne-flux faces stay NaN

% ---------- Plot ----------
figure('Name','Effective Yields (Linear Scale)','Units','normalized','Position',[0.1 0.1 0.8 0.5]);

subplot(1,2,1);
patch(transpose(X), transpose(Y), transpose(Z), effYield_D, ...
      'FaceAlpha',1,'EdgeAlpha',0.3);
title('Effective Yield — D^+','Interpreter','none');
xlabel('X [m]'); ylabel('Y [m]'); zlabel('Z [m]');
cb1 = colorbar('eastoutside'); ylabel(cb1,'Effective Yield');
axis equal tight; view(30,30);
set(gca,'FontSize',10);

subplot(1,2,2);
patch(transpose(X), transpose(Y), transpose(Z), effYield_Ne, ...
      'FaceAlpha',1,'EdgeAlpha',0.3);
title('Effective Yield — Ne (all charge states)','Interpreter','none');
xlabel('X [m]'); ylabel('Y [m]'); zlabel('Z [m]');
cb2 = colorbar('eastoutside'); ylabel(cb2,'Effective Yield');
axis equal tight; view(30,30);
set(gca,'FontSize',10);

% ---------- Optional: same color scale for fair comparison ----------
allEff = [effYield_D(:); effYield_Ne(:)];
