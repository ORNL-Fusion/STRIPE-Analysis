...
data=readmatrix('Targets_Ne9+.txt');
yields_data=readmatrix('yields_Ne9+.csv');
potential_data=data(:,1);
ne_data=data(:,11);
te_data=data(:,3);
v_data=data(:,5);
ero_data=[0;yields_data(:,9)].*ne_data.*v_data;
...
patch(transpose(X),transpose(Y),transpose(Z),[0;yields_data(:,9)],'FaceAlpha',1,'EdgeAlpha', 0.3)%,impacts(surface)
title('Yields')
