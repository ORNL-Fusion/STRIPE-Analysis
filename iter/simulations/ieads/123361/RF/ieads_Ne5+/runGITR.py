...
import shutil
import numpy as np
target = np.loadtxt('Targets_Ne5+.txt', dtype='float',skiprows=1,delimiter=',')
...
for i in range(0, 21159):
    for j in range(5,6):
        ...
        theta = target[i,9]
        ni = target[i,10]
        print("Running case for location ", i, " charge ", j)
        ...
        impacts.iter_case_Ne(surface_potential = surface_potential, charge = j, Tee=te, Tii = ti, n=ni,btot = btot,theta=theta, sheath_factor = 1.0)
