...
data=readmatrix('Targets_Ne2+.txt');
yields_data=readmatrix('yields_Ne2+.csv');
potential_data=data(:,1);
ni_data=data(:,11);
te_data=data(:,3);
v_data=data(:,5);
ero_data=[0;yields_data(:,2)].*ni_data.*v_data;
...
patch(transpose(X),transpose(Y),transpose(Z),[0;yields_data(:,2)],'FaceAlpha',1,'EdgeAlpha', 0.3)%,impacts(surface)
title('Yields')
