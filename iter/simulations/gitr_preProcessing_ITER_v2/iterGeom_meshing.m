...
% Load STL Geometry
model = createpde;
importGeometry(model,'iter_extrusion.stl');
figure(2)
...
load('final_ITERGeom.mat');  % Assumes 'planes' variable is present

figure;
patch(transpose(X), transpose(Y), transpose(Z), 'FaceAlpha', 1, 'EdgeColor', 'none');
...
figure;
patch(transpose(X), transpose(Y), transpose(Z), ne_surf, 'FaceAlpha', 1, 'EdgeColor', 'none');
title('Potential applied only on ITER geometry')
xlabel('X'); ylabel('Y'); zlabel('Z');
