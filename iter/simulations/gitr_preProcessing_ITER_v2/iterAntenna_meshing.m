...
% solps_to_cartesian_v3;
% extrap_ITER_profile_test;
% clear var X Y;
read_efit_data;
% ITER_antenna_data
antenna_ITER_new;
...
refine_inds = find(area > 0.0005 | aspect_ratio > 50 | longest_side > 0.1);
...
%% --- Load Data ---
comsol_data = readmatrix('../comsol/VDC_IMAS123361.txt');
xx = comsol_data(:,1);
yy = comsol_data(:,2);
zz = comsol_data(:,3);
emag = comsol_data(:,5); % Field magnitude
...
% potential_surf_thermal = 3.185.*ti_surf; 
...
% yields_total=0.5.*ones(size(centroid_z));
density_total=nD_surf;
flow_total=vDmag_surf;
% erosion_flux_total=0.5.*nD_surf.*vDmag_surf;
erosion_flux_total=readmatrix('total_erosion_flux.txt');
yields_surf_total=readmatrix('total_yield_sum_from_species.txt');

surface_variables=table(potential_surf,ne_surf,te_surf,ti_surf,vNeMag_surf,Bmag_surf,Br_surf,Bt_surf,Bz_surf,theta,nNe_surf);% for Neon ions
% surface_variables=table(potential_surf,ne_surf,te_surf,ti_surf,vDmag_surf,Bmag_surf,Br_surf,Bt_surf,Bz_surf,theta,nD_surf); % for D+ ions
writetable(surface_variables,'Targets.txt')

surf_fields=table(centroid_x, centroid_y, centroid_z, potential_surf, nD_surf, vDmag_surf, yields_surf_total, erosion_flux_total);
...
erosion_flux=erosion_flux_total;
