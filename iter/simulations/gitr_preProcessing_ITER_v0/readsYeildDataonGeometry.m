...
ne_data = 0.02.*(0.23+0.26+0.205378+0.205378+0.067+0.02*0.031+0.013).*data(:,2);
ne_data_ele= data(:,2);
te_data = data(:,3);
v_data = data(:,5);
flux_data =abs([0;yields].*ne_data .* v_data);
ero_data =  flux_data;
test = data(:,10);
...
figure; 
patch(transpose(X), transpose(Y), transpose(Z), ne_data_ele, ...
    'FaceAlpha', 1, 'EdgeAlpha', 0.3);
